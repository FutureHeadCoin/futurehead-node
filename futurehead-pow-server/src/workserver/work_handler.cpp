...
std::atomic<unsigned> futurehead_pow_server::job::job_id_dispenser{ 1 };

futurehead_pow_server::job::job ()
{
	job_id = job_id_dispenser.fetch_add (1);
}

/** Currently we support work on a single device, hence a single-thread pool */
futurehead_pow_server::work_handler::work_handler (futurehead_pow_server::config const & config_a, std::shared_ptr<spdlog::logger> const & logger_a)
    : config (config_a)
    , logger (logger_a)
    , pool (config.devices.size ())
{
	...
	{
		std::shared_ptr<futurehead_pow::driver> driver;
		if (device.type == futurehead_pow_server::config::device::device_type::cpu)
		{
			driver = std::make_shared<futurehead_pow::cpp_driver> ();
		}
		else if (device.type == futurehead_pow_server::config::device::device_type::gpu)
		{
			driver = std::make_shared<futurehead_pow::opencl_driver> ();
		}
...
futurehead_pow_server::work_handler::~work_handler ()
{
	pool.stop ();
}

void futurehead_pow_server::work_handler::handle_queue_request (std::function<void(std::string)> response_handler)
{
...
void futurehead_pow_server::work_handler::handle_queue_delete_request (std::function<void(std::string)> response_handler)
{
	boost::property_tree::ptree response;
...
void futurehead_pow_server::work_handler::handle_request_async (std::string body, std::function<void(std::string)> response_handler)
{
	...
	try
	{
			...
			boost::asio::post (pool, [this, correlation_id, response_handler, create_error_response, attach_correlation_id] {
				...
				if (!jobs.empty ())
				{
					...
					try
					{
						...
						// TODO: set difficulty when handling request once the futurehead-pow API is set
						//       as well as set_memory based on request?
						// driver->difficulty_set(futurehead_pow::reverse ((1ULL << 40) - 1));
						// driver->memory_set (...);

						// TODO: futurehead-pow API currently takes a 64 bit integer, same with the nonce
						...
						{
							// TODO: use values from futurehead-pow / to_multiplier ()
		...
		else if (action && *action == "work_validate")
		{
			...
			// TODO: update to use futurehead_pow::passes when the API is done
			bool passes (true);

			boost::property_tree::ptree response;
			response.put ("valid", passes ? "1" : "0");

			// TODO: add these when the futurehead_pow API is done

...
if [ -n "$DOCKER_PASSWORD" ]; then
    ...
    if [[ "$GITHUB_WORKFLOW" = "Develop" ]]; then
        "$scripts"/custom-timeout.sh 30 docker push "futureheadcurrency/futurehead-env:base"
        "$scripts"/custom-timeout.sh 30 docker push "futureheadcurrency/futurehead-env:gcc"
        "$scripts"/custom-timeout.sh 30 docker push "futureheadcurrency/futurehead-env:clang"
        echo "Deployed futurehead-env"
        exit 0
    else
        tags=()

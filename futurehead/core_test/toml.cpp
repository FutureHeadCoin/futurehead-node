...
TEST (toml, diff)
{
	futurehead::tomlconfig defaults, other;
...
{
	futurehead::tomlconfig t;
	boost::filesystem::path data_path (".");
	futurehead::daemon_config c (data_path);
...
TEST (toml, rpc_config_deserialize_defaults)
{
	...
	futurehead::tomlconfig t;
	t.read (ss);
	futurehead::rpc_config conf;
	futurehead::rpc_config defaults;
...
TEST (toml, daemon_config_deserialize_defaults)
{
	...
	futurehead::tomlconfig t;
	t.read (ss);
	futurehead::daemon_config conf;
	futurehead::daemon_config defaults;
	conf.deserialize_toml (t);
...
TEST (toml, base_override)
{
	...
	ASSERT_TRUE (t.get_error () == futurehead::error_config::missing_value);
	t.get_error ().clear ();

	// Query uint16 that's too big, make sure we have an error
	t.get_required<uint16_t> ("node.too_big", port);
	ASSERT_TRUE (t.get_error ());
	ASSERT_TRUE (t.get_error () == futurehead::error_config::invalid_value);
}

TEST (toml, put)
{
	futurehead::tomlconfig config;
	futurehead::tomlconfig config_node;
	// Overwrite value and add to child node
	config_node.put ("port", "7155");//TO-CHANGE
	config.put_child ("node", config_node);
	uint16_t port;
	config.get_required<uint16_t> ("node.port", port);
	ASSERT_EQ (port, 7155);//TO-CHANGE
...
TEST (toml, daemon_config_deserialize_no_defaults)
{
	...
	preconfigured_representatives = ["fpsc_3arg3asgtigae3xckabaaewkx3bzsh7nwz7jkmjos79ihyaxwphhm6qgjps4"]
	receive_minimum = "999"
...
TEST (toml, daemon_config_deserialize_errors)
{
	...
	futurehead::tomlconfig toml2;
	toml2.read (ss_frontiers_confirmation);
	futurehead::daemon_config conf2;
	...
	ASSERT_EQ (conf2.node.frontiers_confirmation, futurehead::frontiers_confirmation_mode::invalid);
}
...
TEST (toml, daemon_read_config)
{
	...
	futurehead::daemon_config config;
	...
	ASSERT_FALSE (boost::filesystem::exists (futurehead::get_node_toml_config_path (path)));
	ASSERT_FALSE (futurehead::read_node_config_toml (path, config));
	{
		auto error = futurehead::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
		ASSERT_EQ (error.get_message (), expected_message1);
	}
	{
		auto error = futurehead::read_node_config_toml (path, config, invalid_overrides2);
		ASSERT_TRUE (error);
	...
	futurehead::tomlconfig toml;
	toml.write (futurehead::get_node_toml_config_path (path));

	// Reading when there is a config file
	ASSERT_TRUE (boost::filesystem::exists (futurehead::get_node_toml_config_path (path)));
	ASSERT_FALSE (futurehead::read_node_config_toml (path, config));
	{
		auto error = futurehead::read_node_config_toml (path, config, invalid_overrides1);
		ASSERT_TRUE (error);
...
TEST (toml, tls_config_deserialize_no_defaults)
{
	...
	futurehead::tomlconfig toml;
	toml.read (ss);
	futurehead::tls_config conf;
	futurehead::tls_config defaults;
	conf.deserialize_toml (toml);
...
TEST (toml, tls_config_defaults)
{
	...
	ASSERT_EQ (conf.enable_https, defaults.enable_https);

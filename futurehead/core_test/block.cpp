...
{
	futurehead::private_key prv (0);
	auto pub (futurehead::pub_key (prv));
	futurehead::uint256_union message (0);
...
TEST (transaction_block, empty)
{
	...
	futurehead::send_block block (0, 1, 13, key1.prv, key1.pub, 2);
	auto hash (block.hash ());
	ASSERT_FALSE (futurehead::validate_message (key1.pub, hash, block.signature));
	block.signature.bytes[32] ^= 0x1;
	ASSERT_TRUE (futurehead::validate_message (key1.pub, hash, block.signature));
}

TEST (block, send_serialize)
{
	futurehead::send_block block1 (0, 1, 2, futurehead::keypair ().prv, 4, 5);
	...
	ASSERT_NE (0, size);
	futurehead::bufferstream stream2 (data, size);
	bool error (false);
	futurehead::send_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (block, send_serialize_json)
{
	...
	bool error (false);
	futurehead::send_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	futurehead::receive_block block1 (0, 1, futurehead::keypair ().prv, 3, 4);
	futurehead::keypair key1;
	std::vector<uint8_t> bytes;
	{
		futurehead::vectorstream stream1 (bytes);
	...
	bool error (false);
	futurehead::receive_block block2 (error, stream2);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
TEST (block, receive_serialize_json)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	futurehead::receive_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
TEST (block, open_serialize_json)
{
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	futurehead::open_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	futurehead::change_block block1 (0, 1, futurehead::keypair ().prv, 3, 4);
	std::string string1;
	block1.serialize_json (string1);
	ASSERT_NE (0, string1.size ());
	boost::property_tree::ptree tree1;
	...
	boost::property_tree::read_json (istream, tree1);
	bool error (false);
	futurehead::change_block block2 (error, tree1);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	futurehead::uint512_union input (futurehead::uint512_t (0));
	std::string text;
	input.encode_hex (text);
	futurehead::uint512_union output;
...
{
	futurehead::uint512_union input (futurehead::uint512_t (1));
	std::string text;
	input.encode_hex (text);
	futurehead::uint512_union output;
	auto error (output.decode_hex (text));
...
{
	futurehead::uint512_union input (futurehead::uint512_t (1000));
	std::string text;
	input.encode_hex (text);
	text[5] = '!';
	futurehead::uint512_union output;
...
{
	futurehead::uint512_union input (std::numeric_limits<futurehead::uint512_t>::max ());
	std::string text;
	input.encode_hex (text);
	futurehead::uint512_union output;
	auto error (output.decode_hex (text));
	ASSERT_FALSE (error);
	ASSERT_EQ (input, output);
	ASSERT_EQ (futurehead::uint512_t ("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"), output.number ());
}
...
TEST (send_block, deserialize)
{
	...
	ASSERT_EQ (futurehead::send_block::size, bytes.size ());
	futurehead::bufferstream stream2 (bytes.data (), bytes.size ());
	bool error (false);
	futurehead::send_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (receive_block, deserialize)
{
	...
	ASSERT_EQ (futurehead::receive_block::size, bytes.size ());
	futurehead::bufferstream stream2 (bytes.data (), bytes.size ());
	bool error (false);
	futurehead::receive_block block2 (error, stream2);
	ASSERT_FALSE (error);
...
TEST (open_block, deserialize)
{
	...
	ASSERT_EQ (futurehead::open_block::size, bytes.size ());
	futurehead::bufferstream stream (bytes.data (), bytes.size ());
	bool error (false);
	futurehead::open_block block2 (error, stream);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	futurehead::change_block block1 (1, 2, futurehead::keypair ().prv, 4, 5);
	ASSERT_EQ (block1.hash (), block1.hash ());
	std::vector<uint8_t> bytes;
	{
		futurehead::vectorstream stream1 (bytes);
	...
	ASSERT_EQ (futurehead::change_block::size, bytes.size ());
	auto data (bytes.data ());
	auto size (bytes.size ());
	ASSERT_NE (nullptr, data);
	ASSERT_NE (0, size);
	futurehead::bufferstream stream2 (data, size);
	bool error (false);
	futurehead::change_block block2 (error, stream2);
	ASSERT_FALSE (error);
	ASSERT_EQ (block1, block2);
}
...
{
	futurehead::frontier_req request1;
	request1.start = 1;
	request1.age = 2;
	request1.count = 3;
	std::vector<uint8_t> bytes;
	...
	ASSERT_FALSE (error);
	futurehead::frontier_req request2 (error, stream, header);
	ASSERT_FALSE (error);
...
TEST (block, publish_req_serialization)
{
	...
	ASSERT_FALSE (error);
	futurehead::publish req2 (error, stream2, header);
	ASSERT_FALSE (error);
	ASSERT_EQ (req, req2);
...
{
	futurehead::send_block block (0, 1, 2, futurehead::keypair ().prv, 4, 5);
	ASSERT_EQ (block.difficulty (), futurehead::work_difficulty (block.work_version (), block.root (), block.block_work ()));
}
...
TEST (state_block, serialization)
{
	futurehead::keypair key1;
	futurehead::keypair key2;
	futurehead::state_block block1 (key1.pub, 1, key2.pub, 2, 4, key1.prv, key1.pub, 5);
	ASSERT_EQ (key1.pub, block1.hashables.account);
	ASSERT_EQ (futurehead::block_hash (1), block1.previous ());
	ASSERT_EQ (key2.pub, block1.hashables.representative);
	ASSERT_EQ (futurehead::amount (2), block1.hashables.balance);
	ASSERT_EQ (futurehead::uint256_union (4), block1.hashables.link);
	std::vector<uint8_t> bytes;
	{
		futurehead::vectorstream stream (bytes);
		block1.serialize (stream);
	...
	ASSERT_EQ (futurehead::state_block::size, bytes.size ());
	bool error1 (false);
	futurehead::bufferstream stream (bytes.data (), bytes.size ());
	futurehead::state_block block2 (error1, stream);
	ASSERT_FALSE (error1);
	ASSERT_EQ (block1, block2);
	...
	bool error2 (false);
	futurehead::state_block block3 (error2, tree);
	ASSERT_FALSE (error2);
...
{
	ASSERT_EQ (futurehead::work_version::work_1, futurehead::send_block ().work_version ());
	ASSERT_EQ (futurehead::work_version::work_1, futurehead::receive_block ().work_version ());
	ASSERT_EQ (futurehead::work_version::work_1, futurehead::change_block ().work_version ());
	ASSERT_EQ (futurehead::work_version::work_1, futurehead::open_block ().work_version ());
	ASSERT_EQ (futurehead::work_version::work_1, futurehead::state_block ().work_version ());
}
...
TEST (block_uniquer, single)
{
	...
	ASSERT_EQ (*block1, *block2);
	std::weak_ptr<futurehead::state_block> block3 (block2);
...
TEST (block_uniquer, cleanup)
{
	futurehead::keypair key;
	auto block1 (std::make_shared<futurehead::state_block> (0, 0, 0, 0, 0, key.prv, key.pub, 0));
	auto block2 (std::make_shared<futurehead::state_block> (0, 0, 0, 0, 0, key.prv, key.pub, 1));
	futurehead::block_uniquer uniquer;
...
TEST (block_builder, from)
{
	...
	auto block = builder
	             .state ()
	             .account_address ("fpsc_15nhh1kzw3x8ohez6s75wy3jr6dqgq65oaede1fzk5hqxk4j8ehz7iqtb3to")
	             ...
	             .representative_address ("fpsc_1stofnrxuz3cai7ze75o174bpm7scwj9jn3nxsn8ntzg784jf1gzn1jjdkou")
...
TEST (block_builder, zeroed_state_block)
{
	...
	auto zero_block_manual (std::make_shared<futurehead::state_block> (0, 0, 0, 0, 0, key.prv, key.pub, 0));
	...
	ASSERT_FALSE (futurehead::validate_message (key.pub, zero_block_build->hash (), zero_block_build->signature));
}
...
TEST (block_builder, state_missing_rep)
{
	...
	auto block = builder
	             ...
	             .work (0)
	             .build (ec);
	ASSERT_EQ (ec, futurehead::error_common::missing_representative);
}
...
TEST (block_builder, state_errors)
{
	...
	builder.state ().account_hex ("fpsc_bad").build (ec);
	ASSERT_EQ (ec, futurehead::error_common::bad_account_number);

	builder.state ().zero ().account_address ("fpsc_1111111111111111111111111111111111111111111111111111hifc8npp").build (ec);
	ASSERT_NO_ERROR (ec);
}
...
TEST (block_builder, open)
{
	...
	auto block = builder
	             .open ()
	             .account_address ("fpsc_3t6k35gi95xu6tergt6p69ck76ogmitsa8mnijtpxm9fkcm736xtoncuohr3")
	             .representative_address ("fpsc_3t6k35gi95xu6tergt6p69ck76ogmitsa8mnijtpxm9fkcm736xtoncuohr3")
...
TEST (block_builder, open_equality)
{
	...
	// With constructor
	futurehead::keypair key1, key2;
	futurehead::open_block block1 (1, key1.pub, key2.pub, key1.prv, key1.pub, 5);
...
TEST (block_builder, change)
{
	...
	auto block = builder
	             .change ()
	             .representative_address ("fpsc_3rropjiqfxpmrrkooej4qtmm1pueu36f9ghinpho4esfdor8785a455d16nf")
...
TEST (block_builder, change_equality)
{
	...
	// With constructor
	futurehead::keypair key1, key2;
	futurehead::change_block block1 (1, key1.pub, key1.prv, key1.pub, 5);
...
TEST (block_builder, send)
{
	...
	auto block = builder
	             .send ()
	             .destination_address ("fpsc_1gys8r4crpxhp94n4uho5cshaho81na6454qni5gu9n53gksoyy1wcd4udyb")
...
TEST (block_builder, send_equality)
{
	...
	// With constructor
	futurehead::keypair key1, key2;
	futurehead::send_block block1 (1, key1.pub, 2, key1.prv, key1.pub, 5);
...
TEST (block_builder, receive_equality)
{
	...
	// With constructor
	futurehead::keypair key1;
	futurehead::receive_block block1 (1, 2, key1.prv, key1.pub, 5);

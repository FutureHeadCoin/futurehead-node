...
namespace
{
void reset_confirmation_heights (futurehead::block_store & store);
bool is_using_rocksdb (boost::filesystem::path const & data_path, std::error_code & ec);
}

std::string futurehead::error_cli_messages::message (int ev) const
{
	switch (static_cast<futurehead::error_cli> (ev))
	{
		case futurehead::error_cli::generic:
			return "Unknown error";
		case futurehead::error_cli::parse_error:
			return "Coud not parse command line";
		case futurehead::error_cli::invalid_arguments:
			return "Invalid arguments";
		case futurehead::error_cli::unknown_command:
			return "Unknown command";
		case futurehead::error_cli::database_write_error:
			return "Database write error";
		case futurehead::error_cli::reading_config:
			return "Config file read error";
		case futurehead::error_cli::disable_all_network:
			return "Flags --disable_tcp_realtime and --disable_udp cannot be used together";
		case futurehead::error_cli::ambiguous_udp_options:
...
void futurehead::add_node_options (boost::program_options::options_description & description_a)
{
	// clang-format off
...
void futurehead::add_node_flag_options (boost::program_options::options_description & description_a)
{
	// clang-format off
	description_a.add_options()
...
std::error_code futurehead::update_flags (futurehead::node_flags & flags_a, boost::program_options::variables_map const & vm)
{
	std::error_code ec;
...
void database_write_lock_error (std::error_code & ec)
{
	...
	ec = futurehead::error_cli::database_write_error;
}
...
std::error_code futurehead::handle_node_options (boost::program_options::variables_map const & vm)
{
	std::error_code ec;
	boost::filesystem::path data_path = vm.count ("data_path") ? boost::filesystem::path (vm["data_path"].as<std::string> ()) : futurehead::working_path ();
			...
			if (!wallet_id.decode_hex (vm["wallet"].as<std::string> ()))
			{
				...
				if (wallet != nullptr)
				{
					...
					else
					{
						...
						ec = futurehead::error_cli::invalid_arguments;
					}
				}
				else
				{
		...
		try
		{
			...
			if (!ec)
			{
				...
				if (success)
				{
					...
					if (using_rocksdb)
					{
						futurehead::remove_all_files_in_dir (backup_path);
						futurehead::move_all_files_to_dir (source_path, backup_path);
						futurehead::move_all_files_to_dir (vacuum_path, source_path);
			...
			if (account_it != vm.cend ())
			{
				...
				if (!account.decode_account (account_str))
				{
					...
					else
					{
						...
						ec = futurehead::error_cli::generic;
					}
				}
				else
				{
	...
	else if (vm.count ("generate_config"))
	{
		...
		if (valid_type)
		{
			std::cout << "# This is an example configuration file for Futurehead. Visit https://docs.futurehead.org/running-a-node/configuration/ for more information.\n#\n"
			          ...
			          << "# Additional information for notable configuration options is available in https://docs.futurehead.org/running-a-node/configuration/#notable-configuration-options\n";
	...
	else if (vm.count ("diagnostics"))
	{
		...
		key.data.clear ();
		futurehead::send_block send (0, 0, 0, key, 0, 0);
		std::cout << "Testing key derivation function" << std::endl;
		futurehead::raw_key junk1;
		junk1.data.clear ();
		futurehead::uint256_union junk2 (0);
		futurehead::kdf kdf;
		kdf.phs (junk1, "", junk2);
	...
	else if (vm.count ("wallet_create"))
	{
		...
		{
			auto inactive_node = futurehead::default_inactive_node (data_path, vm);
			auto wallet_key = futurehead::random_wallet_id ();
				...
				if (existing != inactive_node->node->wallets.items.end ())
				{
					...
					if (!existing->second->enter_password (transaction, password))
					{
						...
						for (auto i (existing->second->store.begin (transaction)), m (existing->second->store.end ()); i != m; ++i)
						{
							...
							if (futurehead::pub_key (key.as_private_key ()) != account)
							{
						...
						else
						{
							...
							else
							{
								bool error (true);
								{
									...
									auto transaction (node->wallets.tx_begin_write ());
									futurehead::wallet wallet (error, transaction, node->wallets, wallet_id.to_string (), contents.str ());
								}
								if (error)
								{
									std::cerr << "Unable to import wallet\n";
	...
	else if (vm.count ("wallet_list"))
	{
		...
		for (auto i (node->wallets.items.begin ()), n (node->wallets.items.end ()); i != n; ++i)
		{
			...
			{
				std::cout << futurehead::account (j->first).to_account () << '\n';
			}
		}
	}
	...
	{
		ec = futurehead::error_cli::unknown_command;
	}

	return ec;
}

std::unique_ptr<futurehead::inactive_node> futurehead::default_inactive_node (boost::filesystem::path const & path_a, boost::program_options::variables_map const & vm_a)
{
	auto node_flags = futurehead::inactive_node_flag_defaults ();
	futurehead::update_flags (node_flags, vm_a);
	return std::make_unique<futurehead::inactive_node> (path_a, node_flags);
}
...
{
	futurehead::daemon_config config (data_path);
	auto error = futurehead::read_node_config_toml (data_path, config);
	if (!error)
	{
		...
		if (use_rocksdb)
		{
			...
			ec = futurehead::error_cli::database_write_error;
#endif
			return (FUTUREHEAD_ROCKSDB == 1);
		}
	}
	else
	{
		ec = futurehead::error_cli::reading_config;
	}

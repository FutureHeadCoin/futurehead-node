...
futurehead::signature_checker::signature_checker (unsigned num_threads) :
...
void futurehead::signature_checker::verify (futurehead::signature_check_set & check_a)
{
...
void futurehead::signature_checker::stop ()
{
...
void futurehead::signature_checker::flush ()
{
	while (!stopped && tasks_remaining != 0)
		;
}

bool futurehead::signature_checker::verify_batch (const futurehead::signature_check_set & check_a, size_t start_index, size_t size)
{
	futurehead::validate_message_batch (check_a.messages + start_index, check_a.message_lengths + start_index, check_a.pub_keys + start_index, check_a.signatures + start_index, size, check_a.verifications + start_index);
 ...
 */
void futurehead::signature_checker::verify_async (futurehead::signature_check_set & check_a, size_t num_batches, std::promise<void> & promise)
{
...
void futurehead::signature_checker::set_thread_names (unsigned num_threads)
{
	...
	for (auto i = 0u; i < num_threads; ++i)
	{
		boost::asio::post (thread_pool, [& promise = promises[i]]() {
			futurehead::thread_role::set (futurehead::thread_role::name::signature_checking);
			promise.set_value ();
		});
	}

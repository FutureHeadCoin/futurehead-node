...
__kernel void coin_work (__global ulong const * attempt, __global ulong * result_a, __global uchar const * item_a, __global ulong const * difficulty_a)
{
...
futurehead::opencl_environment::opencl_environment (bool & error_a)
{
	...
	{
		futurehead::opencl_platform platform;
		platform.platform = *i;
...
void futurehead::opencl_environment::dump (std::ostream & stream)
{
	auto index (0);
...
futurehead::opencl_work::opencl_work (bool & error_a, futurehead::opencl_config const & config_a, futurehead::opencl_environment & environment_a, futurehead::logger_mt & logger_a) :
config (config_a),
context (0),
attempt_buffer (0),
result_buffer (0),
...
logger (logger_a)
{
	...
	if (!error_a)
	{
		...
		if (!error_a)
		{
			futurehead::random_pool::generate_block (reinterpret_cast<uint8_t *> (rand.s.data ()), rand.s.size () * sizeof (decltype (rand.s)::value_type));
			...
			if (!error_a)
			{
				...
				if (!error_a)
				{
					...
					if (!error_a)
					{
						...
						{
							cl_int item_error (0);
							size_t item_size (sizeof (futurehead::uint256_union));
								...
								if (!error_a)
								{
									...
									if (!error_a)
									{
										...
										if (!error_a)
										{
											...
											kernel = clCreateKernel (program, "coin_work", &kernel_error);
...
futurehead::opencl_work::~opencl_work ()
{
	if (kernel != 0)
	{
...
boost::optional<uint64_t> futurehead::opencl_work::generate_work (futurehead::work_version const version_a, futurehead::root const & root_a, uint64_t const difficulty_a)
{
	std::atomic<int> ticket_l{ 0 };
	return generate_work (version_a, root_a, difficulty_a, ticket_l);
}

boost::optional<uint64_t> futurehead::opencl_work::generate_work (futurehead::work_version const version_a, futurehead::root const & root_a, uint64_t const difficulty_a, std::atomic<int> & ticket_a)
{
	...
	while (futurehead::work_difficulty (version_a, root_a, result) < difficulty_a && !error && ticket_a == ticket_l)
	{
		result = rand.next ();
		...
		{
			cl_int write_error2 = clEnqueueWriteBuffer (queue, item_buffer, false, 0, sizeof (futurehead::root), root_a.bytes.data (), 0, nullptr, nullptr);
...
std::unique_ptr<futurehead::opencl_work> futurehead::opencl_work::create (bool create_a, futurehead::opencl_config const & config_a, futurehead::logger_mt & logger_a)
{
	std::unique_ptr<futurehead::opencl_work> result;
	if (create_a)
	{
		auto error (false);
		...
		if (!error)
		{
			result.reset (new futurehead::opencl_work (error, config_a, environment, logger_a));
			if (error)
			{
				result.reset ();
			}

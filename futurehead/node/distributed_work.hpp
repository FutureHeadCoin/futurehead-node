...
#include <futurehead/boost/beast/http/string_body.hpp>
...
struct work_request final
{
	...
	uint64_t difficulty;
	boost::optional<futurehead::account> const account;
 ...
 */
class distributed_work final : public std::enable_shared_from_this<futurehead::distributed_work>
{
	...
	{
	public:
		peer_request (boost::asio::io_context & io_ctx_a, futurehead::tcp_endpoint const & endpoint_a) :
		endpoint (endpoint_a),
		socket (io_ctx_a)
		{
		}
		std::shared_ptr<request_type> get_prepared_json_request (std::string const &) const;
		futurehead::tcp_endpoint const endpoint;
...
public:
	distributed_work (futurehead::node &, futurehead::work_request const &, std::chrono::seconds const &);
	~distributed_work ();
	void start ();
	void cancel ();
...
private:
	...
	void do_request (futurehead::tcp_endpoint const & endpoint_a);
	/** Send a work_cancel message using a new connection to \p endpoint_a */
	void do_cancel (futurehead::tcp_endpoint const & endpoint_a);
	/** Called on a successful peer response, validates the reply */
	void success (std::string const &, futurehead::tcp_endpoint const &);
	...
	void failure ();
	void handle_failure ();
	void add_bad_peer (futurehead::tcp_endpoint const &);
	...
	std::weak_ptr<futurehead::node> node_w;
	futurehead::work_request request;
	...
	futurehead::timer<std::chrono::milliseconds> elapsed; // logging only

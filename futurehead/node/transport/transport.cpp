...
namespace
{
class callback_visitor : public futurehead::message_visitor
{
public:
	void keepalive (futurehead::keepalive const & message_a) override
	{
		result = futurehead::stat::detail::keepalive;
	}
	void publish (futurehead::publish const & message_a) override
	{
		result = futurehead::stat::detail::publish;
	}
	void confirm_req (futurehead::confirm_req const & message_a) override
	{
		result = futurehead::stat::detail::confirm_req;
	}
	void confirm_ack (futurehead::confirm_ack const & message_a) override
	{
		result = futurehead::stat::detail::confirm_ack;
	}
	void bulk_pull (futurehead::bulk_pull const & message_a) override
	{
		result = futurehead::stat::detail::bulk_pull;
	}
	void bulk_pull_account (futurehead::bulk_pull_account const & message_a) override
	{
		result = futurehead::stat::detail::bulk_pull_account;
	}
	void bulk_push (futurehead::bulk_push const & message_a) override
	{
		result = futurehead::stat::detail::bulk_push;
	}
	void frontier_req (futurehead::frontier_req const & message_a) override
	{
		result = futurehead::stat::detail::frontier_req;
	}
	void node_id_handshake (futurehead::node_id_handshake const & message_a) override
	{
		result = futurehead::stat::detail::node_id_handshake;
	}
	void telemetry_req (futurehead::telemetry_req const & message_a) override
	{
		result = futurehead::stat::detail::telemetry_req;
	}
	void telemetry_ack (futurehead::telemetry_ack const & message_a) override
	{
		result = futurehead::stat::detail::telemetry_ack;
	}
	futurehead::stat::detail result;
};
}

futurehead::endpoint futurehead::transport::map_endpoint_to_v6 (futurehead::endpoint const & endpoint_a)
{
	auto endpoint_l (endpoint_a);
	if (endpoint_l.address ().is_v4 ())
	{
		endpoint_l = futurehead::endpoint (boost::asio::ip::address_v6::v4_mapped (endpoint_l.address ().to_v4 ()), endpoint_l.port ());
	}
	return endpoint_l;
}

futurehead::endpoint futurehead::transport::map_tcp_to_endpoint (futurehead::tcp_endpoint const & endpoint_a)
{
	return futurehead::endpoint (endpoint_a.address (), endpoint_a.port ());
}

futurehead::tcp_endpoint futurehead::transport::map_endpoint_to_tcp (futurehead::endpoint const & endpoint_a)
{
	return futurehead::tcp_endpoint (endpoint_a.address (), endpoint_a.port ());
}

futurehead::transport::channel::channel (futurehead::node & node_a) :
node (node_a)
{
	set_network_version (node_a.network_params.protocol.protocol_version);
}

void futurehead::transport::channel::send (futurehead::message const & message_a, std::function<void(boost::system::error_code const &, size_t)> const & callback_a, futurehead::buffer_drop_policy drop_policy_a)
{
	...
	auto is_droppable_by_limiter = drop_policy_a == futurehead::buffer_drop_policy::limiter;
	...
	if (!is_droppable_by_limiter || !should_drop)
	{
		...
		node.stats.inc (futurehead::stat::type::message, detail, futurehead::stat::dir::out);
	}
	else
	{
		if (callback_a)
		...
		node.stats.inc (futurehead::stat::type::drop, detail, futurehead::stat::dir::out);
...
bool futurehead::transport::reserved_address (futurehead::endpoint const & endpoint_a, bool allow_local_peers)
{
...
futurehead::bandwidth_limiter::bandwidth_limiter (const double limit_burst_ratio_a, const size_t limit_a) :
bucket (limit_a * limit_burst_ratio_a, limit_a)
{
}

bool futurehead::bandwidth_limiter::should_drop (const size_t & message_size_a)
{

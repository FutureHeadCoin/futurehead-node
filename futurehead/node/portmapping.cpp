...
std::string futurehead::port_mapping::get_config_port (std::string const & node_port_a)
{
	return node.config.external_port != 0 ? std::to_string (node.config.external_port) : node_port_a;
}

void futurehead::port_mapping::refresh_devices ()
{
...
futurehead::endpoint futurehead::port_mapping::external_address ()
{
	futurehead::endpoint result_l (boost::asio::ip::address_v6{}, 0);
	futurehead::lock_guard<std::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([](auto const & p) { return p.enabled; }))
	{
		if (protocol.external_port != 0)
		{
			result_l = futurehead::endpoint (protocol.external_address, protocol.external_port);
		}
	}
	return result_l;
}

void futurehead::port_mapping::refresh_mapping ()
{
	...
	if (on)
	{
		...
		for (auto & protocol : protocols | boost::adaptors::filtered ([](auto const & p) { return p.enabled; }))
		{
			...
			auto upnp_description = std::string ("Futurehead Node (") + network_params.network.get_current_network_as_string () + ")";
...
bool futurehead::port_mapping::check_mapping ()
{
...
void futurehead::port_mapping::check_mapping_loop ()
{
	refresh_devices ();
...
void futurehead::port_mapping::stop ()
{
	on = false;
	futurehead::lock_guard<std::mutex> guard_l (mutex);
	for (auto & protocol : protocols | boost::adaptors::filtered ([](auto const & p) { return p.enabled; }))
...
futurehead::upnp_state::~upnp_state ()
{
	if (devices)
	{
...
futurehead::upnp_state & futurehead::upnp_state::operator= (futurehead::upnp_state && other_a)
{
	if (this == &other_a)
	{
		return *this;

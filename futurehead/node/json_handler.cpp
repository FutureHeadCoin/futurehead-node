...
namespace
{
void construct_json (futurehead::container_info_component * component, boost::property_tree::ptree & parent);
using ipc_json_handler_no_arg_func_map = std::unordered_map<std::string, std::function<void(futurehead::json_handler *)>>;
...
bool block_confirmed (futurehead::node & node, futurehead::transaction & transaction, futurehead::block_hash const & hash, bool include_active, bool include_only_confirmed);
const char * epoch_as_string (futurehead::epoch);
}

futurehead::json_handler::json_handler (futurehead::node & node_a, futurehead::node_rpc_config const & node_rpc_config_a, std::string const & body_a, std::function<void(std::string const &)> const & response_a, std::function<void()> stop_callback_a) :
body (body_a),
node (node_a),
response (response_a),
...
std::function<void()> futurehead::json_handler::create_worker_task (std::function<void(std::shared_ptr<futurehead::json_handler> const &)> const & action_a)
{
...
void futurehead::json_handler::process_request (bool unsafe_a)
{
	try
	{
		...
		else
		{
			...
			else if (action == "kfpsc_from_raw")
			{
				mfuturehead_from_raw (futurehead::kxrb_ratio);
			}
			else if (action == "kfpsc_to_raw")
			{
				mfuturehead_to_raw (futurehead::kxrb_ratio);
			}
			else if (action == "fpsc_from_raw")
			{
				mfuturehead_from_raw (futurehead::xrb_ratio);
			}
			else if (action == "fpsc_to_raw")
			{
				mfuturehead_to_raw (futurehead::xrb_ratio);
			}
			else if (action == "mfpsc_from_raw")
			{
				mfuturehead_from_raw ();
			}
			else if (action == "mfpsc_to_raw")
			{
				mfuturehead_to_raw ();
			}
...
void futurehead::json_handler::response_errors ()
{
...
std::shared_ptr<futurehead::wallet> futurehead::json_handler::wallet_impl ()
{
	if (!ec)
	{
		...
		if (!wallet.decode_hex (wallet_text))
		{
			...
			{
				ec = futurehead::error_common::wallet_not_found;
			}
		}
		else
		{
			ec = futurehead::error_common::bad_wallet_number;
		}
	}
	return nullptr;
}

bool futurehead::json_handler::wallet_locked_impl (futurehead::transaction const & transaction_a, std::shared_ptr<futurehead::wallet> wallet_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = futurehead::error_common::wallet_locked;
			result = true;
		}
	}
	return result;
...
bool futurehead::json_handler::wallet_account_impl (futurehead::transaction const & transaction_a, std::shared_ptr<futurehead::wallet> wallet_a, futurehead::account const & account_a)
{
	bool result (false);
	if (!ec)
	{
		...
		{
			ec = futurehead::error_common::account_not_found_wallet;
		}
	}
	return result;
}

futurehead::account futurehead::json_handler::account_impl (std::string account_text, std::error_code ec_a)
{
	...
	if (!ec)
	{
		...
		{
			// futurehead- and xrb- prefixes are deprecated
...
futurehead::account_info futurehead::json_handler::account_info_impl (futurehead::transaction const & transaction_a, futurehead::account const & account_a)
{
	...
	if (!ec)
	{
		...
		{
			ec = futurehead::error_common::account_not_found;
...
futurehead::amount futurehead::json_handler::amount_impl ()
{
	futurehead::amount result (0);
	if (!ec)
	{
		std::string amount_text (request.get<std::string> ("amount"));
		if (result.decode_dec (amount_text))
		{
			ec = futurehead::error_common::invalid_amount;
		}
	}
	return result;
}

std::shared_ptr<futurehead::block> futurehead::json_handler::block_impl (bool signature_work_required)
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	std::shared_ptr<futurehead::block> result{ nullptr };
	if (!ec)
	{
		...
		else
		{
			...
			{
				ec = futurehead::error_blocks::invalid_block;
			}
		}
		if (!ec)
		{
			...
			result = futurehead::deserialize_block_json (block_l);
			if (result == nullptr)
			{
				ec = futurehead::error_blocks::invalid_block;
			}
...
futurehead::block_hash futurehead::json_handler::hash_impl (std::string search_text)
{
	...
	if (!ec)
	{
		...
		{
			ec = futurehead::error_blocks::invalid_block_hash;
		}
	}
	return result;
}

futurehead::amount futurehead::json_handler::threshold_optional_impl ()
{
	futurehead::amount result (0);
	...
	if (!ec && threshold_text.is_initialized ())
	{
		...
		{
			ec = futurehead::error_common::bad_threshold;
		}
	}
	return result;
}

uint64_t futurehead::json_handler::work_optional_impl ()
{
	uint64_t result (0);
	boost::optional<std::string> work_text (request.get_optional<std::string> ("work"));
	if (!ec && work_text.is_initialized ())
	{
		if (futurehead::from_string_hex (work_text.get (), result))
		{
			ec = futurehead::error_common::bad_work_format;
		}
	}
	return result;
}

uint64_t futurehead::json_handler::difficulty_optional_impl (futurehead::work_version const version_a)
{
	...
	{
		if (futurehead::from_string_hex (difficulty_text.get (), difficulty))
		{
			ec = futurehead::error_rpc::bad_difficulty_format;
		}
	}
	return difficulty;
}

uint64_t futurehead::json_handler::difficulty_ledger (futurehead::block const & block_a)
{
	futurehead::block_details details (futurehead::epoch::epoch_0, false, false, false);
	bool details_found (false);
	auto transaction (node.store.tx_begin_read ());
	// Previous block find
	std::shared_ptr<futurehead::block> block_previous (nullptr);
	...
	if (!link.is_zero () && !details.is_send)
	{
		...
		if (block_link != nullptr && node.store.pending_exists (transaction, futurehead::pending_key (block_a.account (), link)))
		{
	...
	return details_found ? futurehead::work_threshold (block_a.work_version (), details) : node.default_difficulty (block_a.work_version ());
}

double futurehead::json_handler::multiplier_optional_impl (futurehead::work_version const version_a, uint64_t & difficulty)
{
	double multiplier (1.);
	...
	if (!ec && multiplier_text.is_initialized ())
	{
		...
		{
			difficulty = futurehead::difficulty::from_multiplier (multiplier, node.default_difficulty (version_a));
		}
		else
		{
			ec = futurehead::error_rpc::bad_multiplier_format;
		}
	}
	return multiplier;
}

futurehead::work_version futurehead::json_handler::work_version_optional_impl (futurehead::work_version const default_a)
{
	futurehead::work_version result = default_a;
	boost::optional<std::string> version_text (request.get_optional<std::string> ("version"));
	if (!ec && version_text.is_initialized ())
	{
		if (*version_text == futurehead::to_string (futurehead::work_version::work_1))
		{
			result = futurehead::work_version::work_1;
		}
		else
		{
			ec = futurehead::error_rpc::bad_work_version;
		}
	}
	return result;
}
...
uint64_t futurehead::json_handler::count_impl ()
{
	uint64_t result (0);
	if (!ec)
	{
		...
		{
			ec = futurehead::error_common::invalid_count;
		}
	}
	return result;
}

uint64_t futurehead::json_handler::count_optional_impl (uint64_t result)
{
...
uint64_t futurehead::json_handler::offset_optional_impl (uint64_t result)
{
	...
	if (!ec && offset_text.is_initialized ())
	{
		...
		{
			ec = futurehead::error_rpc::invalid_offset;
		}
	}
	return result;
}

void futurehead::json_handler::account_balance ()
{
...
void futurehead::json_handler::account_block_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::account_create ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			const bool generate_work = rpc_l->request.get<bool> ("work", true);
			futurehead::account new_key;
			...
			if (index_text.is_initialized ())
			{
				...
				{
					rpc_l->ec = futurehead::error_common::invalid_index;
				}
				else
				{
			...
			if (!rpc_l->ec)
			{
				...
				{
					rpc_l->ec = futurehead::error_common::wallet_locked;
				}
			}
		}
...
void futurehead::json_handler::account_get ()
{
	...
	{
		ec = futurehead::error_common::bad_public_key;
	}
	response_errors ();
}

void futurehead::json_handler::account_info ()
{
	auto account (account_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			std::string balance;
			futurehead::uint128_union (info.balance).encode_dec (balance);
...
void futurehead::json_handler::account_key ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::account_list ()
{
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), j (wallet->store.end ()); i != j; ++i)
		{
			...
			entry.put ("", futurehead::account (i->first).to_account ());
...
void futurehead::json_handler::account_move ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!source.decode_hex (source_text))
			{
				...
				if (existing != rpc_l->node.wallets.items.end ())
				{
					...
					std::vector<futurehead::public_key> accounts;
				...
				{
					rpc_l->ec = futurehead::error_rpc::source_not_found;
				}
			}
			else
			{
				rpc_l->ec = futurehead::error_rpc::bad_source;
			}
		}
		rpc_l->response_errors ();
	}));
...
void futurehead::json_handler::account_remove ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
...
void futurehead::json_handler::account_representative ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::account_representative_set ()
{
	node.worker.push_task (create_worker_task ([work_generation_enabled = node.work_generation_enabled ()](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		auto account (rpc_l->account_impl ());
		std::string representative_text (rpc_l->request.get<std::string> ("representative"));
		auto representative (rpc_l->account_impl (representative_text, futurehead::error_rpc::bad_representative_number));
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && work)
			{
				...
				if (!rpc_l->ec)
				{
					...
					if (!rpc_l->ec)
					{
						futurehead::block_details details (info.epoch (), false, false, false);
						if (futurehead::work_difficulty (futurehead::work_version::work_1, info.head, work) < futurehead::work_threshold (futurehead::work_version::work_1, details))
						{
							rpc_l->ec = futurehead::error_common::invalid_work;
						}
					}
				}
			}
			else if (!rpc_l->ec) // work == 0
			{
				...
				{
					rpc_l->ec = futurehead::error_common::disabled_work_generation;
				}
			}
			if (!rpc_l->ec)
			{
				...
				wallet->change_async (
				account, representative, [response_a, response_data](std::shared_ptr<futurehead::block> block) {
					if (block != nullptr)
					{
...
void futurehead::json_handler::account_weight ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::accounts_balances ()
{
...
void futurehead::json_handler::accounts_create ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		...
		if (!rpc_l->ec)
		{
			...
			{
				futurehead::account new_key (wallet->deterministic_insert (generate_work));
				if (!new_key.is_zero ())
				{
					boost::property_tree::ptree entry;
...
void futurehead::json_handler::accounts_frontiers ()
{
...
void futurehead::json_handler::accounts_pending ()
{
	...
	for (auto & accounts : request.get_child ("accounts"))
	{
		...
		if (!ec)
		{
			...
			for (auto i (node.store.pending_begin (transaction, futurehead::pending_key (account, 0))), n (node.store.pending_end ()); i != n && futurehead::pending_key (i->first).account == account && peers_l.size () < count; ++i)
			{
			...
			if (sorting && !simple)
			{
				if (source)
				{
					peers_l.sort ([](const auto & child1, const auto & child2) -> bool {
						return child1.second.template get<futurehead::uint128_t> ("amount") > child2.second.template get<futurehead::uint128_t> ("amount");
					});
				}
				else
				{
					peers_l.sort ([](const auto & child1, const auto & child2) -> bool {
						return child1.second.template get<futurehead::uint128_t> ("") > child2.second.template get<futurehead::uint128_t> ("");
					});
				}
			}
...
void futurehead::json_handler::active_difficulty ()
{
	auto include_trend (request.get<bool> ("include_trend", false));
	auto multiplier_active = node.active.active_multiplier ();
	auto default_difficulty (node.default_difficulty (futurehead::work_version::work_1));
	response_l.put ("network_minimum", futurehead::to_string_hex (default_difficulty));
	response_l.put ("network_current", futurehead::to_string_hex (futurehead::difficulty::from_multiplier (multiplier_active, default_difficulty)));
	...
	if (include_trend)
	{
		...
		{
			boost::property_tree::ptree entry;
			entry.put ("", futurehead::to_string (multiplier_l));
...
void futurehead::json_handler::available_supply ()
{
	auto genesis_balance (node.balance (node.network_params.ledger.genesis_account)); // Cold storage genesis
	auto landing_balance (node.balance (futurehead::account ("059F68AAB29DE0D3A27443625C7EA9CDDB6517A8B76FE37727EF6A4D76832AD5"))); // Active unavailable account
	auto faucet_balance (node.balance (futurehead::account ("8E319CE6F3025E5B2DF66DA7AB1467FE48F1679C13DD43BFDB29FA2E9FC40D3B"))); // Faucet account
	auto burned_balance ((node.balance_pending (futurehead::account (0))).second); // Burning 0 account
...
void futurehead::json_handler::block_info ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			futurehead::account account (block->account ().is_zero () ? block->sideband ().account : block->account ());
			...
			{
				auto subtype (futurehead::state_subtype (block->sideband ().details));
				response_l.put ("subtype", subtype);
			}
		}
		else
		{
			ec = futurehead::error_blocks::not_found;
		}
	}
	response_errors ();
}

void futurehead::json_handler::block_confirm ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block_l != nullptr)
		{
			...
			else
			{
				...
				futurehead::election_status status{ block_l, 0, std::chrono::duration_cast<std::chrono::milliseconds> (std::chrono::system_clock::now ().time_since_epoch ()), std::chrono::duration_values<std::chrono::milliseconds>::zero (), 0, 1, 0, futurehead::election_status_type::active_confirmation_height };
				{
				...
				if (auto state = dynamic_cast<futurehead::state_block *> (block_l.get ()))
				{
...
void futurehead::json_handler::blocks ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	...
	for (boost::property_tree::ptree::value_type & hashes : request.get_child ("hashes"))
	{
		if (!ec)
		{
			...
			{
				ec = futurehead::error_blocks::bad_hash_number;
			}
		}
	}
...
void futurehead::json_handler::blocks_info ()
{
	const bool pending = request.get<bool> ("pending", false);
	const bool source = request.get<bool> ("source", false);
	const bool json_block_l = request.get<bool> ("json_block", false);
			...
			if (!hash.decode_hex (hash_text))
			{
				...
				if (block != nullptr)
				{
					...
					if (pending)
					{
						...
						{
							exists = node.store.pending_exists (transaction, futurehead::pending_key (destination, hash));
						}
						entry.put ("pending", exists ? "1" : "0");
					}
					if (source)
					{
						futurehead::block_hash source_hash (node.ledger.block_source (transaction, *block));
...
void futurehead::json_handler::block_account ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::block_count ()
{
...
void futurehead::json_handler::block_count_type ()
{
	auto transaction (node.store.tx_begin_read ());
	futurehead::block_counts count (node.store.block_count (transaction));
...
void futurehead::json_handler::block_create ()
{
	std::string type (request.get<std::string> ("type"));
	futurehead::wallet_id wallet (0);
	// Default to work_1 if not specified
	auto work_version (work_version_optional_impl (futurehead::work_version::work_1));
	...
	{
		representative = account_impl (representative_text.get (), futurehead::error_rpc::bad_representative_number);
	}
	...
	{
		destination = account_impl (destination_text.get (), futurehead::error_rpc::bad_destination);
	}
	futurehead::block_hash source (0);
	...
	if (!ec && source_text.is_initialized ())
	{
		...
		{
			ec = futurehead::error_rpc::bad_source;
		}
	}
	...
	{
		ec = futurehead::error_common::disabled_work_generation;
	}
	...
	if (!ec && key_text.is_initialized ())
	{
		...
		{
			ec = futurehead::error_common::bad_private_key;
		}
	}
	...
	if (!ec && previous_text.is_initialized ())
	{
		...
		{
			ec = futurehead::error_rpc::bad_previous;
		}
	}
	...
	if (!ec && balance_text.is_initialized ())
	{
		...
		{
			ec = futurehead::error_rpc::invalid_balance;
		}
	}
	...
	if (!ec && link_text.is_initialized ())
	{
		if (link.decode_account (link_text.get ()))
		{
			...
			{
				ec = futurehead::error_rpc::bad_link;
			}
		}
	}
	else
	...
	if (!ec)
	{
		...
		auto block_response_put_l = [rpc_l, this](futurehead::block const & block_a) {
			boost::property_tree::ptree response_l;
			response_l.put ("hash", block_a.hash ().to_string ());
			response_l.put ("difficulty", futurehead::to_string_hex (block_a.difficulty ()));
			bool json_block_l = request.get<bool> ("json_block", false);
			if (json_block_l)
			{
				boost::property_tree::ptree block_node_l;
		...
		auto get_callback_l = [rpc_l, block_response_put_l](std::shared_ptr<futurehead::block> block_a) {
			...
			return [block_a, rpc_l, block_response_put_l](boost::optional<uint64_t> const & work_a) {
				if (block_a != nullptr)
				{
					...
					{
						rpc_l->ec = futurehead::error_common::failure_work_generation;
					}
				}
				else
				{
					rpc_l->ec = futurehead::error_common::generic;
				}
				if (rpc_l->ec)
				{
					rpc_l->response_errors ();
		...
		if (prv.data != 0)
		{
			futurehead::account pub (futurehead::pub_key (prv.as_private_key ()));
			...
			else if (previous_text.is_initialized () && balance_text.is_initialized () && type == "send")
			{
				...
				{
					ec = futurehead::error_rpc::block_create_balance_mismatch;
				}
			}
			...
			if (!ec && account_text.is_initialized ())
			{
				if (account != pub)
				{
					ec = futurehead::error_rpc::block_create_public_key_mismatch;
				}
			}
			futurehead::block_builder builder_l;
			std::shared_ptr<futurehead::block> block_l{ nullptr };
			futurehead::root root_l;
			...
			if (type == "state")
			{
				...
				{
					ec = futurehead::error_rpc::block_create_requirements_state;
				}
			}
			else if (type == "open")
			{
				...
				{
					ec = futurehead::error_rpc::block_create_requirements_open;
				}
			}
			else if (type == "receive")
			{
				...
				{
					ec = futurehead::error_rpc::block_create_requirements_receive;
				}
			}
			else if (type == "change")
			{
				...
				{
					ec = futurehead::error_rpc::block_create_requirements_change;
				}
			}
			else if (type == "send")
			{
				if (destination != 0 && previous != 0 && balance != 0 && amount != 0)
				{
					...
					{
						ec = futurehead::error_common::insufficient_balance;
					}
				}
				else
				{
					ec = futurehead::error_rpc::block_create_requirements_send;
				}
			}
			else
			{
				ec = futurehead::error_blocks::invalid_type;
			}
			if (!ec && (!ec_build || ec_build == futurehead::error_common::missing_work))
			{
				if (work == 0)
				{
					...
					node.work_generate (work_version, root_l, difficulty_l, get_callback_l (block_l), futurehead::account (pub));
				}
				else
				{
		...
		{
			ec = futurehead::error_rpc::block_create_key_required;
		}
	}
	// Because of callback
	if (ec)
...
void futurehead::json_handler::block_hash ()
{
...
void futurehead::json_handler::bootstrap ()
{
	...
	{
		uint16_t port;
		if (!futurehead::parse_port (port_text, port))
		{
			if (!node.flags.disable_legacy_bootstrap)
			{
				std::string bootstrap_id (request.get<std::string> ("id", ""));
				node.bootstrap_initiator.bootstrap (futurehead::endpoint (address, port), true, bypass_frontier_confirmation, bootstrap_id);
				response_l.put ("success", "");
			}
			else
			{
				ec = futurehead::error_rpc::disabled_bootstrap_legacy;
			}
		}
		else
		{
			ec = futurehead::error_common::invalid_port;
		}
	}
	else
	{
		ec = futurehead::error_common::invalid_ip_address;
	}
	response_errors ();
}

void futurehead::json_handler::bootstrap_any ()
{
...
void futurehead::json_handler::bootstrap_lazy ()
{
	auto hash (hash_impl ());
	const bool force = request.get<bool> ("force", false);
	if (!ec)
	{
		...
		{
			ec = futurehead::error_rpc::disabled_bootstrap_lazy;
		}
	}
	response_errors ();
}
 ...
 */
void futurehead::json_handler::bootstrap_status ()
{
	...
	{
		futurehead::lock_guard<std::mutex> connections_lock (node.bootstrap_initiator.connections->mutex);
	...
	{
		futurehead::lock_guard<std::mutex> attempts_lock (node.bootstrap_initiator.attempts.bootstrap_attempts_mutex);
...
void futurehead::json_handler::chain (bool successors)
{
...
void futurehead::json_handler::confirmation_active ()
{
...
void futurehead::json_handler::confirmation_height_currently_processing ()
{
	...
	{
		ec = futurehead::error_rpc::confirmation_height_not_processing;
	}
	response_errors ();
}

void futurehead::json_handler::confirmation_history ()
{
	boost::property_tree::ptree elections;
...
void futurehead::json_handler::confirmation_info ()
{
	...
	if (!root.decode_hex (root_text))
	{
		...
		if (election != nullptr && !election->confirmed ())
		{
			...
			for (auto i (tally_l.begin ()), n (tally_l.end ()); i != n; ++i)
			{
				...
				if (representatives)
				{
					std::multimap<futurehead::uint128_t, futurehead::account, std::greater<futurehead::uint128_t>> representatives;
					for (auto ii (election->last_votes.begin ()), nn (election->last_votes.end ()); ii != nn; ++ii)
					{
						if (i->second->hash () == ii->second.hash)
						{
							futurehead::account const & representative (ii->first);
		...
		{
			ec = futurehead::error_rpc::confirmation_not_found;
		}
	}
	else
	{
		ec = futurehead::error_rpc::invalid_root;
	}
	response_errors ();
}

void futurehead::json_handler::confirmation_quorum ()
{
...
void futurehead::json_handler::database_txn_tracker ()
{
	...
	{
		ec = futurehead::error_common::tracking_not_enabled;
	}

	response_errors ();
}

void futurehead::json_handler::delegators ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::delegators_count ()
{
	auto account (account_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::deterministic_key ()
{
	...
	if (!seed.data.decode_hex (seed_text))
	{
		try
		{
			...
			futurehead::private_key prv = futurehead::deterministic_key (seed, index);
			futurehead::public_key pub (futurehead::pub_key (prv));
		...
		{
			ec = futurehead::error_common::invalid_index;
		}
	}
	else
	{
		ec = futurehead::error_common::bad_seed;
	}
	response_errors ();
}
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void futurehead::json_handler::epoch_upgrade ()
{
	futurehead::epoch epoch (futurehead::epoch::invalid);
	uint8_t epoch_int (request.get<uint8_t> ("epoch"));
	switch (epoch_int)
	{
		case 1:
			epoch = futurehead::epoch::epoch_1;
			break;
		case 2:
			epoch = futurehead::epoch::epoch_2;
			break;
		default:
			break;
	}
	if (epoch != futurehead::epoch::invalid)
	{
		...
		if (!ec && threads_text.is_initialized ())
		{
			...
			{
				ec = futurehead::error_rpc::invalid_threads_count;
			}
		}
		std::string key_text (request.get<std::string> ("key"));
		futurehead::private_key prv;
		if (!prv.decode_hex (key_text))
		{
			if (futurehead::pub_key (prv) == node.ledger.epoch_signer (node.ledger.epoch_link (epoch)))
			{
			...
			{
				ec = futurehead::error_rpc::invalid_epoch_signer;
			}
		}
		else
		{
	...
	{
		ec = futurehead::error_rpc::invalid_epoch;
	}
	response_errors ();
}

void futurehead::json_handler::frontiers ()
{
...
void futurehead::json_handler::account_count ()
{
...
namespace
{
class history_visitor : public futurehead::block_visitor
{
public:
	history_visitor (futurehead::json_handler & handler_a, bool raw_a, futurehead::transaction & transaction_a, boost::property_tree::ptree & tree_a, futurehead::block_hash const & hash_a, std::vector<futurehead::public_key> const & accounts_filter_a) :
	handler (handler_a),
	raw (raw_a),
	...
	void send_block (futurehead::send_block const & block_a)
	{
	...
	void receive_block (futurehead::receive_block const & block_a)
	{
	...
	void open_block (futurehead::open_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "open");
	...
	void change_block (futurehead::change_block const & block_a)
	{
	...
	void state_block (futurehead::state_block const & block_a)
	{
		if (raw)
		{
			tree.put ("type", "state");
	...
	bool should_ignore_account (futurehead::public_key const & account)
	{
		bool ignore (false);
	...
	futurehead::json_handler & handler;
	bool raw;
	futurehead::transaction & transaction;
	boost::property_tree::ptree & tree;
	futurehead::block_hash const & hash;
	futurehead::network_params network_params;
	std::vector<futurehead::public_key> const & accounts_filter;
};
}

void futurehead::json_handler::account_history ()
{
	std::vector<futurehead::public_key> accounts_to_filter;
		...
		while (block != nullptr && count > 0)
		{
			...
			else
			{
				...
				if (!entry.empty ())
				{
					...
					if (output_raw)
					{
						entry.put ("work", futurehead::to_string_hex (block->block_work ()));
...
void futurehead::json_handler::keepalive ()
{
	if (!ec)
	{
		std::string address_text (request.get<std::string> ("address"));
...
void futurehead::json_handler::key_create ()
{
	futurehead::keypair pair;
...
void futurehead::json_handler::key_expand ()
{
	std::string key_text (request.get<std::string> ("key"));
	futurehead::private_key prv;
	if (!prv.decode_hex (key_text))
...
void futurehead::json_handler::ledger ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		futurehead::account start (0);
		boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
		...
		if (modified_since_text.is_initialized ())
		{
			...
			{
				ec = futurehead::error_rpc::invalid_timestamp;
			}
		}
		const bool sorting = request.get<bool> ("sorting", false);
		const bool representative = request.get<bool> ("representative", false);
		...
		{
			std::vector<std::pair<futurehead::uint128_union, futurehead::account>> ledger_l;
			for (auto i (node.store.latest_begin (transaction, start)), n (node.store.latest_end ()); i != n; ++i)
			{
				futurehead::account_info const & info (i->second);
				futurehead::uint128_union balance (info.balance);
			...
			for (auto i (ledger_l.begin ()), n (ledger_l.end ()); i != n && accounts.size () < count; ++i)
			{
				...
				{
					futurehead::account const & account (i->second);
					boost::property_tree::ptree response_a;
					if (pending)
					{
						auto account_pending (node.ledger.account_pending (transaction, account));
...
void futurehead::json_handler::mfuturehead_from_raw (futurehead::uint128_t ratio)
{
...
void futurehead::json_handler::mfuturehead_to_raw (futurehead::uint128_t ratio)
{
	auto amount (amount_impl ());
	if (!ec)
	{
		...
		{
			ec = futurehead::error_common::invalid_amount_big;
		}
	}
	response_errors ();
}
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void futurehead::json_handler::node_id ()
{
	if (!ec)
	{
 ...
 * @warning This is an internal/diagnostic RPC, do not rely on its interface being stable
 */
void futurehead::json_handler::node_id_delete ()
{
	response_l.put ("deprecated", "1");
	response_errors ();
}

void futurehead::json_handler::password_change ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
...
void futurehead::json_handler::password_enter ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
...
void futurehead::json_handler::password_valid (bool wallet_locked)
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::peers ()
{
	boost::property_tree::ptree peers_l;
	...
	for (auto i (peers_list.begin ()), n (peers_list.end ()); i != n; ++i)
	{
		...
		if (peer_details)
		{
			...
			pending_tree.put ("type", channel->get_type () == futurehead::transport::transport_type::tcp ? "tcp" : "udp");
...
void futurehead::json_handler::pending ()
{
	auto account (account_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
...
void futurehead::json_handler::pending_exists ()
{
	auto hash (hash_impl ());
	const bool include_active = request.get<bool> ("include_active", false);
	const bool include_only_confirmed = request.get<bool> ("include_only_confirmed", false);
...
void futurehead::json_handler::payment_begin ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		...
		if (!id.decode_hex (id_text))
		{
			...
			{
				auto transaction (rpc_l->node.wallets.tx_begin_write ());
				std::shared_ptr<futurehead::wallet> wallet (existing->second);
				if (wallet->store.valid_password (transaction))
				{
					...
					{
						rpc_l->ec = futurehead::error_rpc::payment_unable_create_account;
					}
				}
				else
				{
			...
			{
				rpc_l->ec = futurehead::error_common::wallet_not_found;
			}
		}
		else
		{
			rpc_l->ec = futurehead::error_common::bad_wallet_number;
		}
		rpc_l->response_errors ();
	}));
}

void futurehead::json_handler::payment_init ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
...
void futurehead::json_handler::payment_end ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			{
				ec = futurehead::error_rpc::payment_account_balance;
			}
		}
	}
	response_errors ();
...
void futurehead::json_handler::payment_wait ()
{
	...
	if (!ec)
	{
		...
		{
			{
				auto observer (std::make_shared<futurehead::json_payment_observer> (node, response, account, amount));
		...
		{
			ec = futurehead::error_rpc::bad_timeout;
		}
	}
	if (ec)
	{
...
void futurehead::json_handler::process ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		...
		if (!rpc_l->ec && block->type () == futurehead::block_type::state)
		{
			std::string subtype_text (rpc_l->request.get<std::string> ("subtype", ""));
			if (!subtype_text.empty ())
			{
				std::shared_ptr<futurehead::state_block> block_state (std::static_pointer_cast<futurehead::state_block> (block));
				...
				{
					rpc_l->ec = futurehead::error_process::gap_previous;
				}
				else
				{
					...
					if (subtype_text == "send")
					{
						...
						{
							rpc_l->ec = futurehead::error_rpc::invalid_subtype_balance;
						}
					...
					else if (subtype_text == "open")
					{
						...
						{
							rpc_l->ec = futurehead::error_rpc::invalid_subtype_previous;
						}
					}
					...
					else if (subtype_text == "epoch")
					{
						...
						{
							rpc_l->ec = futurehead::error_rpc::invalid_subtype_epoch_link;
						}
					}
					else
					{
						rpc_l->ec = futurehead::error_rpc::invalid_subtype;
					}
				}
			}
		}
		if (!rpc_l->ec)
		{
			if (!futurehead::work_validate_entry (*block))
			{
				...
				switch (result.code)
				{
					...
					case futurehead::process_result::fork:
					{
						...
						{
							rpc_l->ec = futurehead::error_process::fork;
						}
						break;
					}
					case futurehead::process_result::insufficient_work:
					{
						rpc_l->ec = futurehead::error_process::insufficient_work;
						break;
					}
					default:
					{
						rpc_l->ec = futurehead::error_process::other;
						break;
					}
				}
			}
			...
			{
				rpc_l->ec = futurehead::error_blocks::work_low;
			}
		}
		rpc_l->response_errors ();
	}));
...
void futurehead::json_handler::receive ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	auto hash (hash_impl ("block"));
	if (!ec)
	{
		...
		if (!ec)
		{
			...
			if (block != nullptr)
			{
				if (node.store.pending_exists (block_transaction, futurehead::pending_key (account, hash)))
				{
					auto work (work_optional_impl ());
					if (!ec && work)
					{
						futurehead::account_info info;
						futurehead::root head;
						futurehead::epoch epoch = block->sideband ().details.epoch;
						...
						futurehead::block_details details (epoch, false, true, false);
						if (futurehead::work_difficulty (futurehead::work_version::work_1, head, work) < futurehead::work_threshold (futurehead::work_version::work_1, details))
						{
							ec = futurehead::error_common::invalid_work;
						}
					}
					else if (!ec) // && work == 0
					{
					...
					if (!ec)
					{
						...
						futurehead::account representative (wallet->store.representative (wallet_transaction));
						bool generate_work (work == 0); // Disable work generation if "work" option is provided
						auto response_a (response);
						wallet->receive_async (
						std::move (block), representative, node.network_params.ledger.genesis_amount, [response_a](std::shared_ptr<futurehead::block> block_a) {
							if (block_a != nullptr)
							{
								boost::property_tree::ptree response_l;
				...
				{
					ec = futurehead::error_process::unreceivable;
				}
			}
			else
			{
...
void futurehead::json_handler::receive_minimum ()
{
	if (!ec)
	{
...
void futurehead::json_handler::receive_minimum_set ()
{
	auto amount (amount_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::representatives ()
{
	auto count (count_optional_impl ());
	if (!ec)
	{
		...
		if (!sorting) // Simple
		{
			std::map<futurehead::account, futurehead::uint128_t> ordered (rep_amounts.begin (), rep_amounts.end ());
		...
		{
			std::vector<std::pair<futurehead::uint128_t, std::string>> representation;
...
void futurehead::json_handler::representatives_online ()
{
...
void futurehead::json_handler::republish ()
{
	...
	if (!ec && sources_text.is_initialized ())
	{
		...
		{
			ec = futurehead::error_rpc::invalid_sources;
		}
	}
	...
	if (!ec && destinations_text.is_initialized ())
	{
		...
		{
			ec = futurehead::error_rpc::invalid_destinations;
		}
	}
	auto hash (hash_impl ());
	if (!ec)
	{
		...
		if (block != nullptr)
		{
			std::deque<std::shared_ptr<futurehead::block>> republish_bundle;
			for (auto i (0); !hash.is_zero () && i < count; ++i)
			{
				block = node.store.block_get (transaction, hash);
				if (sources != 0) // Republish source chain
				{
					futurehead::block_hash source (node.ledger.block_source (transaction, *block));
				...
				if (destinations != 0) // Republish destination chain
				{
					...
					if (!destination.is_zero ())
					{
						if (!node.store.pending_exists (transaction, futurehead::pending_key (destination, hash)))
						{
							futurehead::block_hash previous (node.ledger.latest (transaction, destination));
...
void futurehead::json_handler::search_pending ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::search_pending_all ()
{
	if (!ec)
	{
...
void futurehead::json_handler::send ()
{
	auto wallet (wallet_impl ());
	auto amount (amount_impl ());
	...
	auto source (account_impl (source_text, futurehead::error_rpc::bad_source));
	std::string destination_text (request.get<std::string> ("destination"));
	auto destination (account_impl (destination_text, futurehead::error_rpc::bad_destination));
	if (!ec)
	{
		auto work (work_optional_impl ());
		...
		if (!ec)
		{
			...
			if (!ec && work)
			{
				futurehead::block_details details (info.epoch (), true, false, false);
				if (futurehead::work_difficulty (futurehead::work_version::work_1, info.head, work) < futurehead::work_threshold (futurehead::work_version::work_1, details))
				{
					ec = futurehead::error_common::invalid_work;
				}
		...
		if (!ec)
		{
			...
			wallet->send_async (
			source, destination, amount.number (), [balance, amount, response_a, response_data](std::shared_ptr<futurehead::block> block_a) {
				if (block_a != nullptr)
				{
				...
				else
				{
					...
					{
						std::error_code ec (futurehead::error_common::insufficient_balance);
...
void futurehead::json_handler::sign ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	// Retrieving hash
	futurehead::block_hash hash (0);
	...
	{
		ec = futurehead::error_rpc::sign_hash_disabled;
	}
	if (!ec)
	{
		futurehead::raw_key prv;
		...
		if (prv.data != 0)
		{
			...
			futurehead::signature signature (futurehead::sign_message (prv, pub, hash));
...
void futurehead::json_handler::stats ()
{
	...
	{
		ec = futurehead::error_rpc::invalid_missing_type;
	}
	if (!ec && use_sink)
	{
...
void futurehead::json_handler::stats_clear ()
{
	node.stats.clear ();
	response_l.put ("success", "");
	std::stringstream ostream;
...
void futurehead::json_handler::stop ()
{
	response_l.put ("success", "");
	response_errors ();
	if (!ec)
...
void futurehead::json_handler::telemetry ()
{
	auto rpc_l (shared_from_this ());
	...
	if (address_text.is_initialized () || port_text.is_initialized ())
	{
		...
		if (address_text.is_initialized () && port_text.is_initialized ())
		{
			...
			if (!futurehead::parse_port (*port_text, port))
			{
				...
				if (!futurehead::parse_address (*address_text, address))
				{
					futurehead::endpoint endpoint (address, port);
					if (address.is_loopback () && port == rpc_l->node.network.endpoint ().port ())
					{
						// Requesting telemetry metrics locally
						auto telemetry_data = futurehead::local_telemetry_data (rpc_l->node.ledger.cache, rpc_l->node.network, rpc_l->node.config.bandwidth_limit, rpc_l->node.network_params, rpc_l->node.startup_time, rpc_l->node.active.active_difficulty (), rpc_l->node.node_id);

						futurehead::jsonconfig config_l;
					...
					{
						channel = node.network.find_channel (futurehead::transport::map_endpoint_to_v6 (endpoint));
						if (!channel)
						{
							ec = futurehead::error_rpc::peer_not_found;
						}
					}
				}
				else
		...
		{
			ec = futurehead::error_rpc::requires_port_and_address;
		}
			...
			if (node.telemetry)
			{
				node.telemetry->get_metrics_single_peer_async (channel, [rpc_l](auto const & telemetry_response_a) {
					if (!telemetry_response_a.error)
					{
						...
						{
							rpc_l->ec = futurehead::error_rpc::generic;
						}
					}
					else
					{
		...
		if (node.telemetry)
		{
			...
			if (output_raw)
			{
				...
				for (auto & telemetry_metrics : telemetry_responses)
				{
					...
					{
						ec = futurehead::error_rpc::generic;
					}
				}
			...
			else
			{
				...
				auto average_telemetry_metrics = futurehead::consolidate_telemetry_data (telemetry_datas);
...
void futurehead::json_handler::unchecked ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	if (!ec)
...
void futurehead::json_handler::unchecked_clear ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
...
void futurehead::json_handler::unchecked_get ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto hash (hash_impl ());
	if (!ec)
...
void futurehead::json_handler::unchecked_keys ()
{
	const bool json_block_l = request.get<bool> ("json_block", false);
	auto count (count_optional_impl ());
	futurehead::block_hash key (0);
	...
	if (!ec && hash_text.is_initialized ())
	{
		...
		{
			ec = futurehead::error_rpc::bad_key;
		}
	}
	if (!ec)
	{
		boost::property_tree::ptree unchecked;
		auto transaction (node.store.tx_begin_read ());
		for (auto i (node.store.unchecked_begin (transaction, futurehead::unchecked_key (key, 0))), n (node.store.unchecked_end ()); i != n && unchecked.size () < count; ++i)
		{
			boost::property_tree::ptree entry;
			futurehead::unchecked_info const & info (i->second);
...
void futurehead::json_handler::unopened ()
{
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	futurehead::account start (1); // exclude burn account by default
	boost::optional<std::string> account_text (request.get_optional<std::string> ("account"));
	if (account_text.is_initialized ())
	{
		start = account_impl (account_text.get ());
	...
	{
		auto transaction (node.store.tx_begin_read ());
		auto iterator (node.store.pending_begin (transaction, futurehead::pending_key (start, 0)));
		auto end (node.store.pending_end ());
		futurehead::account current_account (start);
		futurehead::uint128_t current_account_sum{ 0 };
		boost::property_tree::ptree accounts;
		while (iterator != end && accounts.size () < count)
		{
			futurehead::pending_key key (iterator->first);
			futurehead::account account (key.account);
			futurehead::pending_info info (iterator->second);
			if (node.store.account_exists (transaction, account))
			{
				if (account.number () == std::numeric_limits<futurehead::uint256_t>::max ())
				{
					break;
				}
				// Skip existing accounts
				iterator = node.store.pending_begin (transaction, futurehead::pending_key (account.number () + 1, 0));
			}
			else
			{
...
void futurehead::json_handler::validate_account_number ()
{
	auto account (account_impl ());
	(void)account;
...
void futurehead::json_handler::wallet_add ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = futurehead::error_common::bad_private_key;
			}
		}
		rpc_l->response_errors ();
	}));
...
void futurehead::json_handler::wallet_add_watch ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
			...
			if (wallet->store.valid_password (transaction))
			{
				for (auto & accounts : rpc_l->request.get_child ("accounts"))
				{
					...
					if (!rpc_l->ec)
					{
						...
						{
							rpc_l->ec = futurehead::error_common::bad_public_key;
						}
					}
				}
				if (!rpc_l->ec)
...
void futurehead::json_handler::wallet_info ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		futurehead::uint128_t balance (0);
		futurehead::uint128_t pending (0);
		uint64_t count (0);
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			futurehead::key_type key_type (wallet->store.key_type (i->second));
			if (key_type == futurehead::key_type::deterministic)
			{
				deterministic_count++;
			}
			else if (key_type == futurehead::key_type::adhoc)
			{
				adhoc_count++;
			}
			count++;
...
void futurehead::json_handler::wallet_balances ()
{
	auto wallet (wallet_impl ());
	auto threshold (threshold_optional_impl ());
	if (!ec)
	{
		...
		{
			futurehead::account const & account (i->first);
			futurehead::uint128_t balance = node.ledger.account_balance (block_transaction, account);
			if (balance >= threshold.number ())
			{
				boost::property_tree::ptree entry;
				futurehead::uint128_t pending = node.ledger.account_pending (block_transaction, account);
				entry.put ("balance", balance.convert_to<std::string> ());
...
void futurehead::json_handler::wallet_change_seed ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		if (!rpc_l->ec)
		{
			...
			if (!seed.data.decode_hex (seed_text))
			{
				...
				if (wallet->store.valid_password (transaction))
				{
					futurehead::public_key account (wallet->change_seed (transaction, seed, count));
					rpc_l->response_l.put ("success", "");
			...
			{
				rpc_l->ec = futurehead::error_common::bad_seed;
			}
		}
		rpc_l->response_errors ();
	}));
...
void futurehead::json_handler::wallet_contains ()
{
	auto account (account_impl ());
	auto wallet (wallet_impl ());
	if (!ec)
...
void futurehead::json_handler::wallet_create ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		futurehead::raw_key seed;
		...
		if (!rpc_l->ec)
		{
			...
			{
				rpc_l->ec = futurehead::error_common::wallet_lmdb_max_dbs;
			}
			if (!rpc_l->ec && seed_text.is_initialized ())
			{
				auto transaction (rpc_l->node.wallets.tx_begin_write ());
				futurehead::public_key account (wallet->change_seed (transaction, seed));
				rpc_l->response_l.put ("last_restored_account", account.to_account ());
				auto index (wallet->store.deterministic_index_get (transaction));
				debug_assert (index > 0);
				rpc_l->response_l.put ("restored_count", std::to_string (index));
...
void futurehead::json_handler::wallet_destroy ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
...
void futurehead::json_handler::wallet_export ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::wallet_frontiers ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::wallet_history ()
{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			if (!node.store.account_get (block_transaction, account, info))
			{
				...
				while (timestamp >= modified_since && !hash.is_zero ())
				{
					...
					{
						boost::property_tree::ptree entry;
						std::vector<futurehead::public_key> no_filter;
...
void futurehead::json_handler::wallet_key_valid ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::wallet_ledger ()
{
	const bool representative = request.get<bool> ("representative", false);
	const bool weight = request.get<bool> ("weight", false);
	const bool pending = request.get<bool> ("pending", false);
...
void futurehead::json_handler::wallet_lock ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		futurehead::raw_key empty;
		empty.data.clear ();
...
void futurehead::json_handler::wallet_pending ()
{
	auto wallet (wallet_impl ());
	auto count (count_optional_impl ());
	auto threshold (threshold_optional_impl ());
	...
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			futurehead::account const & account (i->first);
			boost::property_tree::ptree peers_l;
			for (auto ii (node.store.pending_begin (block_transaction, futurehead::pending_key (account, 0))), nn (node.store.pending_end ()); ii != nn && futurehead::pending_key (ii->first).account == account && peers_l.size () < count; ++ii)
			{
				futurehead::pending_key key (ii->first);
				if (block_confirmed (node, block_transaction, key.hash, include_active, include_only_confirmed))
				{
					...
					{
						futurehead::pending_info info (ii->second);
						if (info.amount.number () >= threshold.number ())
						{
							if (source || min_version)
							{
...
void futurehead::json_handler::wallet_representative ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::wallet_representative_set ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		std::string representative_text (rpc_l->request.get<std::string> ("representative"));
		...
		if (!rpc_l->ec)
		{
			...
			if (!rpc_l->ec && update_existing_accounts)
			{
				...
				{
					wallet->change_async (
					account, representative, [](std::shared_ptr<futurehead::block>) {}, 0, false);
				}
			}
		}
		rpc_l->response_errors ();
...
void futurehead::json_handler::wallet_republish ()
{
	auto wallet (wallet_impl ());
	auto count (count_impl ());
	if (!ec)
...
void futurehead::json_handler::wallet_seed ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::wallet_work_get ()
{
	auto wallet (wallet_impl ());
	if (!ec)
	{
		...
		for (auto i (wallet->store.begin (transaction)), n (wallet->store.end ()); i != n; ++i)
		{
			...
			(void)error_work;
			works.put (account.to_account (), futurehead::to_string_hex (work));
		}
...
void futurehead::json_handler::work_generate ()
{
	boost::optional<futurehead::account> account;
	...
	if (!ec)
	{
		...
		if (!ec && (difficulty > node.max_work_generate_difficulty (work_version) || difficulty < futurehead::work_threshold_entry (work_version)))
		{
			ec = futurehead::error_rpc::difficulty_limit;
		}
		...
		if (!ec && request.count ("block"))
		{
			...
			if (block != nullptr)
			{
				if (hash != block->root ())
				{
					ec = futurehead::error_rpc::block_root_mismatch;
				}
				...
				{
					ec = futurehead::error_rpc::block_work_version_mismatch;
				}
				// Difficulty calculation
				...
				{
					ec = futurehead::error_rpc::block_work_enough;
				}
			}
		}
		if (!ec && response_l.empty ())
		{
			...
			auto callback = [rpc_l, hash, work_version, this](boost::optional<uint64_t> const & work_a) {
				if (work_a)
				{
					...
					response_l.put ("work", futurehead::to_string_hex (work));
					std::stringstream ostream;
					auto result_difficulty (futurehead::work_difficulty (work_version, hash, work));
					response_l.put ("difficulty", futurehead::to_string_hex (result_difficulty));
					auto result_multiplier = futurehead::difficulty::to_multiplier (result_difficulty, node.default_difficulty (work_version));
					response_l.put ("multiplier", futurehead::to_string (result_multiplier));
					boost::property_tree::write_json (ostream, response_l);
					rpc_l->response (ostream.str ());
				}
				else
			...
			if (!use_peers)
			{
				if (node.local_work_generation_enabled ())
				{
					...
					if (error)
					{
						ec = futurehead::error_common::failure_work_generation;
					}
				}
				else
				{
					ec = futurehead::error_common::disabled_local_work_generation;
				}
			}
			else
			{
...
void futurehead::json_handler::work_cancel ()
{
	auto hash (hash_impl ());
	if (!ec)
	{
...
void futurehead::json_handler::work_get ()
{
	auto wallet (wallet_impl ());
	auto account (account_impl ());
	if (!ec)
...
void futurehead::json_handler::work_set ()
{
	node.worker.push_task (create_worker_task ([](std::shared_ptr<futurehead::json_handler> const & rpc_l) {
		auto wallet (rpc_l->wallet_impl ());
		auto account (rpc_l->account_impl ());
...
void futurehead::json_handler::work_validate ()
{
	auto hash (hash_impl ());
	auto work (work_optional_impl ());
	// Default to work_1 if not specified
	...
	if (!ec)
	{
		...
		response_l.put ("valid_receive", (result_difficulty >= futurehead::work_threshold (work_version, futurehead::block_details (futurehead::epoch::epoch_2, false, true, false))) ? "1" : "0");
		response_l.put ("difficulty", futurehead::to_string_hex (result_difficulty));
		auto result_multiplier = futurehead::difficulty::to_multiplier (result_difficulty, node.default_difficulty (work_version));
		response_l.put ("multiplier", futurehead::to_string (result_multiplier));
	}
...
void futurehead::json_handler::work_peer_add ()
{
	std::string address_text = request.get<std::string> ("address");
	std::string port_text = request.get<std::string> ("port");
	uint16_t port;
...
void futurehead::json_handler::work_peers ()
{
...
void futurehead::json_handler::work_peers_clear ()
{
...
void futurehead::inprocess_rpc_handler::process_request (std::string const &, std::string const & body_a, std::function<void(std::string const &)> response_a)
{
	// Note that if the rpc action is async, the shared_ptr<json_handler> lifetime will be extended by the action handler
	auto handler (std::make_shared<futurehead::json_handler> (node, node_rpc_config, body_a, response_a, [this]() {
		this->stop_callback ();
		this->stop ();
	}));
...
void futurehead::inprocess_rpc_handler::process_request_v2 (rpc_handler_request_params const & params_a, std::string const & body_a, std::function<void(std::shared_ptr<std::string>)> response_a)
{
	std::string body_l = params_a.json_envelope (body_a);
	auto handler (std::make_shared<futurehead::ipc::flatbuffers_handler> (node, ipc_server, nullptr, node.config.ipc_config));
...
void construct_json (futurehead::container_info_component * component, boost::property_tree::ptree & parent)
{
	...
	{
		auto & leaf_info = static_cast<futurehead::container_info_leaf *> (component)->get_info ();
	...
	auto composite = static_cast<futurehead::container_info_composite *> (component);
...
ipc_json_handler_no_arg_func_map create_ipc_json_handler_no_arg_func_map ()
{
	...
	no_arg_funcs.emplace ("account_balance", &futurehead::json_handler::account_balance);
	no_arg_funcs.emplace ("account_block_count", &futurehead::json_handler::account_block_count);
	no_arg_funcs.emplace ("account_count", &futurehead::json_handler::account_count);
	no_arg_funcs.emplace ("account_create", &futurehead::json_handler::account_create);
	no_arg_funcs.emplace ("account_get", &futurehead::json_handler::account_get);
	no_arg_funcs.emplace ("account_history", &futurehead::json_handler::account_history);
	no_arg_funcs.emplace ("account_info", &futurehead::json_handler::account_info);
	no_arg_funcs.emplace ("account_key", &futurehead::json_handler::account_key);
	no_arg_funcs.emplace ("account_list", &futurehead::json_handler::account_list);
	no_arg_funcs.emplace ("account_move", &futurehead::json_handler::account_move);
	no_arg_funcs.emplace ("account_remove", &futurehead::json_handler::account_remove);
	no_arg_funcs.emplace ("account_representative", &futurehead::json_handler::account_representative);
	no_arg_funcs.emplace ("account_representative_set", &futurehead::json_handler::account_representative_set);
	no_arg_funcs.emplace ("account_weight", &futurehead::json_handler::account_weight);
	no_arg_funcs.emplace ("accounts_balances", &futurehead::json_handler::accounts_balances);
	no_arg_funcs.emplace ("accounts_create", &futurehead::json_handler::accounts_create);
	no_arg_funcs.emplace ("accounts_frontiers", &futurehead::json_handler::accounts_frontiers);
	no_arg_funcs.emplace ("accounts_pending", &futurehead::json_handler::accounts_pending);
	no_arg_funcs.emplace ("active_difficulty", &futurehead::json_handler::active_difficulty);
	no_arg_funcs.emplace ("available_supply", &futurehead::json_handler::available_supply);
	no_arg_funcs.emplace ("block_info", &futurehead::json_handler::block_info);
	no_arg_funcs.emplace ("block", &futurehead::json_handler::block_info);
	no_arg_funcs.emplace ("block_confirm", &futurehead::json_handler::block_confirm);
	no_arg_funcs.emplace ("blocks", &futurehead::json_handler::blocks);
	no_arg_funcs.emplace ("blocks_info", &futurehead::json_handler::blocks_info);
	no_arg_funcs.emplace ("block_account", &futurehead::json_handler::block_account);
	no_arg_funcs.emplace ("block_count", &futurehead::json_handler::block_count);
	no_arg_funcs.emplace ("block_count_type", &futurehead::json_handler::block_count_type);
	no_arg_funcs.emplace ("block_create", &futurehead::json_handler::block_create);
	no_arg_funcs.emplace ("block_hash", &futurehead::json_handler::block_hash);
	no_arg_funcs.emplace ("bootstrap", &futurehead::json_handler::bootstrap);
	no_arg_funcs.emplace ("bootstrap_any", &futurehead::json_handler::bootstrap_any);
	no_arg_funcs.emplace ("bootstrap_lazy", &futurehead::json_handler::bootstrap_lazy);
	no_arg_funcs.emplace ("bootstrap_status", &futurehead::json_handler::bootstrap_status);
	no_arg_funcs.emplace ("confirmation_active", &futurehead::json_handler::confirmation_active);
	no_arg_funcs.emplace ("confirmation_height_currently_processing", &futurehead::json_handler::confirmation_height_currently_processing);
	no_arg_funcs.emplace ("confirmation_history", &futurehead::json_handler::confirmation_history);
	no_arg_funcs.emplace ("confirmation_info", &futurehead::json_handler::confirmation_info);
	no_arg_funcs.emplace ("confirmation_quorum", &futurehead::json_handler::confirmation_quorum);
	no_arg_funcs.emplace ("database_txn_tracker", &futurehead::json_handler::database_txn_tracker);
	no_arg_funcs.emplace ("delegators", &futurehead::json_handler::delegators);
	no_arg_funcs.emplace ("delegators_count", &futurehead::json_handler::delegators_count);
	no_arg_funcs.emplace ("deterministic_key", &futurehead::json_handler::deterministic_key);
	no_arg_funcs.emplace ("epoch_upgrade", &futurehead::json_handler::epoch_upgrade);
	no_arg_funcs.emplace ("frontiers", &futurehead::json_handler::frontiers);
	no_arg_funcs.emplace ("frontier_count", &futurehead::json_handler::account_count);
	no_arg_funcs.emplace ("keepalive", &futurehead::json_handler::keepalive);
	no_arg_funcs.emplace ("key_create", &futurehead::json_handler::key_create);
	no_arg_funcs.emplace ("key_expand", &futurehead::json_handler::key_expand);
	no_arg_funcs.emplace ("ledger", &futurehead::json_handler::ledger);
	no_arg_funcs.emplace ("node_id", &futurehead::json_handler::node_id);
	no_arg_funcs.emplace ("node_id_delete", &futurehead::json_handler::node_id_delete);
	no_arg_funcs.emplace ("password_change", &futurehead::json_handler::password_change);
	no_arg_funcs.emplace ("password_enter", &futurehead::json_handler::password_enter);
	no_arg_funcs.emplace ("wallet_unlock", &futurehead::json_handler::password_enter);
	no_arg_funcs.emplace ("payment_begin", &futurehead::json_handler::payment_begin);
	no_arg_funcs.emplace ("payment_init", &futurehead::json_handler::payment_init);
	no_arg_funcs.emplace ("payment_end", &futurehead::json_handler::payment_end);
	no_arg_funcs.emplace ("payment_wait", &futurehead::json_handler::payment_wait);
	no_arg_funcs.emplace ("peers", &futurehead::json_handler::peers);
	no_arg_funcs.emplace ("pending", &futurehead::json_handler::pending);
	no_arg_funcs.emplace ("pending_exists", &futurehead::json_handler::pending_exists);
	no_arg_funcs.emplace ("process", &futurehead::json_handler::process);
	no_arg_funcs.emplace ("receive", &futurehead::json_handler::receive);
	no_arg_funcs.emplace ("receive_minimum", &futurehead::json_handler::receive_minimum);
	no_arg_funcs.emplace ("receive_minimum_set", &futurehead::json_handler::receive_minimum_set);
	no_arg_funcs.emplace ("representatives", &futurehead::json_handler::representatives);
	no_arg_funcs.emplace ("representatives_online", &futurehead::json_handler::representatives_online);
	no_arg_funcs.emplace ("republish", &futurehead::json_handler::republish);
	no_arg_funcs.emplace ("search_pending", &futurehead::json_handler::search_pending);
	no_arg_funcs.emplace ("search_pending_all", &futurehead::json_handler::search_pending_all);
	no_arg_funcs.emplace ("send", &futurehead::json_handler::send);
	no_arg_funcs.emplace ("sign", &futurehead::json_handler::sign);
	no_arg_funcs.emplace ("stats", &futurehead::json_handler::stats);
	no_arg_funcs.emplace ("stats_clear", &futurehead::json_handler::stats_clear);
	no_arg_funcs.emplace ("stop", &futurehead::json_handler::stop);
	no_arg_funcs.emplace ("telemetry", &futurehead::json_handler::telemetry);
	no_arg_funcs.emplace ("unchecked", &futurehead::json_handler::unchecked);
	no_arg_funcs.emplace ("unchecked_clear", &futurehead::json_handler::unchecked_clear);
	no_arg_funcs.emplace ("unchecked_get", &futurehead::json_handler::unchecked_get);
	no_arg_funcs.emplace ("unchecked_keys", &futurehead::json_handler::unchecked_keys);
	no_arg_funcs.emplace ("unopened", &futurehead::json_handler::unopened);
	no_arg_funcs.emplace ("uptime", &futurehead::json_handler::uptime);
	no_arg_funcs.emplace ("validate_account_number", &futurehead::json_handler::validate_account_number);
	no_arg_funcs.emplace ("version", &futurehead::json_handler::version);
	no_arg_funcs.emplace ("wallet_add", &futurehead::json_handler::wallet_add);
	no_arg_funcs.emplace ("wallet_add_watch", &futurehead::json_handler::wallet_add_watch);
	no_arg_funcs.emplace ("wallet_balances", &futurehead::json_handler::wallet_balances);
	no_arg_funcs.emplace ("wallet_change_seed", &futurehead::json_handler::wallet_change_seed);
	no_arg_funcs.emplace ("wallet_contains", &futurehead::json_handler::wallet_contains);
	no_arg_funcs.emplace ("wallet_create", &futurehead::json_handler::wallet_create);
	no_arg_funcs.emplace ("wallet_destroy", &futurehead::json_handler::wallet_destroy);
	no_arg_funcs.emplace ("wallet_export", &futurehead::json_handler::wallet_export);
	no_arg_funcs.emplace ("wallet_frontiers", &futurehead::json_handler::wallet_frontiers);
	no_arg_funcs.emplace ("wallet_history", &futurehead::json_handler::wallet_history);
	no_arg_funcs.emplace ("wallet_info", &futurehead::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_balance_total", &futurehead::json_handler::wallet_info);
	no_arg_funcs.emplace ("wallet_key_valid", &futurehead::json_handler::wallet_key_valid);
	no_arg_funcs.emplace ("wallet_ledger", &futurehead::json_handler::wallet_ledger);
	no_arg_funcs.emplace ("wallet_lock", &futurehead::json_handler::wallet_lock);
	no_arg_funcs.emplace ("wallet_pending", &futurehead::json_handler::wallet_pending);
	no_arg_funcs.emplace ("wallet_representative", &futurehead::json_handler::wallet_representative);
	no_arg_funcs.emplace ("wallet_representative_set", &futurehead::json_handler::wallet_representative_set);
	no_arg_funcs.emplace ("wallet_republish", &futurehead::json_handler::wallet_republish);
	no_arg_funcs.emplace ("wallet_work_get", &futurehead::json_handler::wallet_work_get);
	no_arg_funcs.emplace ("work_generate", &futurehead::json_handler::work_generate);
	no_arg_funcs.emplace ("work_cancel", &futurehead::json_handler::work_cancel);
	no_arg_funcs.emplace ("work_get", &futurehead::json_handler::work_get);
	no_arg_funcs.emplace ("work_set", &futurehead::json_handler::work_set);
	no_arg_funcs.emplace ("work_validate", &futurehead::json_handler::work_validate);
	no_arg_funcs.emplace ("work_peer_add", &futurehead::json_handler::work_peer_add);
	no_arg_funcs.emplace ("work_peers", &futurehead::json_handler::work_peers);
	no_arg_funcs.emplace ("work_peers_clear", &futurehead::json_handler::work_peers_clear);
	return no_arg_funcs;
}
...
const char * epoch_as_string (futurehead::epoch epoch)
{
	switch (epoch)
	{

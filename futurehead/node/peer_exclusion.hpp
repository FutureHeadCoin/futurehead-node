...
class peer_exclusion final
{
	...
	public:
		...
		decltype (std::declval<futurehead::tcp_endpoint> ().address ()) address;
		uint64_t score;
	};
...
public:
	...
	uint64_t add (futurehead::tcp_endpoint const &, size_t const);
	bool check (futurehead::tcp_endpoint const &);
	void remove (futurehead::tcp_endpoint const &);

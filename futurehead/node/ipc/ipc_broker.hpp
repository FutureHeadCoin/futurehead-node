...
namespace ipc
{
	class ipc_config;
	/**
	 * A subscriber represents a live session, and is weakly referenced futurehead::ipc::subscription whenever a subscription is made.
	...
	public:
		 ...
		 */
		virtual void async_send_message (uint8_t const * data_a, size_t length_a, std::function<void(futurehead::error const &)> broadcast_completion_handler_a) = 0;
		...
		virtual futurehead::ipc::payload_encoding get_active_encoding () const = 0;

		/** Get flatbuffer parser instance for this subscriber; create it if necessary */
		std::shared_ptr<flatbuffers::Parser> get_parser (futurehead::ipc::ipc_config const & ipc_config_a);
	...
	{
	public:
		subscription (std::weak_ptr<futurehead::ipc::subscriber> const & subscriber_a, std::shared_ptr<TopicType> const & topic_a) :
	...
	{
	public:
		broker (futurehead::node & node_a);
		/** Starts the broker by setting up observers */
		void start ();
		/** Subscribe to block confirmations */
		void subscribe (std::weak_ptr<futurehead::ipc::subscriber> const & subscriber_a, std::shared_ptr<futureheadapi::TopicConfirmationT> const & confirmation_a);
		/** Subscribe to EventServiceStop notifications for \p subscriber_a. The subscriber must first have called ServiceRegister. */
		void subscribe (std::weak_ptr<futurehead::ipc::subscriber> const & subscriber_a, std::shared_ptr<futureheadapi::TopicServiceStopT> const & service_stop_a);
		...
		void service_register (std::string const & service_name_a, std::weak_ptr<futurehead::ipc::subscriber> const & subscriber_a);
	...
	private:
		...
		void broadcast (std::shared_ptr<futureheadapi::EventConfirmationT> const & confirmation_a);

		futurehead::node & node;
		mutable futurehead::locked<std::vector<subscription<futureheadapi::TopicConfirmationT>>> confirmation_subscribers;
		mutable futurehead::locked<std::vector<subscription<futureheadapi::TopicServiceStopT>>> service_stop_subscribers;
	};
}
}

...
futurehead::ipc::access_permission from_string (std::string permission)
{
	if (permission == "unrestricted")
		return futurehead::ipc::access_permission::unrestricted;
	if (permission == "api_account_weight")
		return futurehead::ipc::access_permission::api_account_weight;
	if (permission == "api_service_register")
		return futurehead::ipc::access_permission::api_service_register;
	if (permission == "api_service_stop")
		return futurehead::ipc::access_permission::api_service_stop;
	if (permission == "api_topic_service_stop")
		return futurehead::ipc::access_permission::api_topic_service_stop;
	if (permission == "api_topic_confirmation")
		return futurehead::ipc::access_permission::api_topic_confirmation;
	if (permission == "account_query")
		return futurehead::ipc::access_permission::account_query;
	if (permission == "epoch_upgrade")
		return futurehead::ipc::access_permission::epoch_upgrade;
	if (permission == "service")
		return futurehead::ipc::access_permission::service;
	if (permission == "wallet")
		return futurehead::ipc::access_permission::wallet;
	if (permission == "wallet_read")
		return futurehead::ipc::access_permission::wallet_read;
	if (permission == "wallet_write")
		return futurehead::ipc::access_permission::wallet_write;
	if (permission == "wallet_seed_change")
		return futurehead::ipc::access_permission::wallet_seed_change;

	return futurehead::ipc::access_permission::invalid;
}
}

void futurehead::ipc::access::set_effective_permissions (futurehead::ipc::access_subject & subject_a, std::shared_ptr<cpptoml::table> const & config_subject_a)
{
	...
	for (auto const & permission : allow_strings_l)
	{
		if (!permission.empty ())
		{
			...
			if (permission_enum != futurehead::ipc::access_permission::invalid)
			{
...
void futurehead::ipc::access::clear ()
{
	users.clear ();
	roles.clear ();
	...
	default_user.permissions.insert (futurehead::ipc::access_permission::api_account_weight);
}

futurehead::error futurehead::ipc::access::deserialize_toml (futurehead::tomlconfig & toml)
{
	...
	if (toml.has_key ("role"))
	{
		auto get_role = [this](std::shared_ptr<cpptoml::table> const & role_a) {
			futurehead::ipc::access_role role;
	...
	if (!error && toml.has_key ("user"))
	{
		auto get_user = [this, &error](std::shared_ptr<cpptoml::table> const & user_a) {
			futurehead::ipc::access_user user;
...
bool futurehead::ipc::access::has_access (std::string const & credentials_a, futurehead::ipc::access_permission permssion_a) const
{
	futurehead::unique_lock<std::mutex> lock (mutex);
	...
	if (user != users.end ())
	{
		...
		if (!permitted)
		{
			permitted = user->second.permissions.find (futurehead::ipc::access_permission::unrestricted) != user->second.permissions.end ();
		}
	}
	return permitted;
}

bool futurehead::ipc::access::has_access_to_all (std::string const & credentials_a, std::initializer_list<futurehead::ipc::access_permission> permissions_a) const
{
	futurehead::unique_lock<std::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
bool futurehead::ipc::access::has_access_to_oneof (std::string const & credentials_a, std::initializer_list<futurehead::ipc::access_permission> permissions_a) const
{
	futurehead::unique_lock<std::mutex> lock (mutex);
	bool permitted = false;
	auto user = users.find (credentials_a);
...
void futurehead::ipc::access_subject::clear ()
{
	permissions.clear ();
}

void futurehead::ipc::access_user::clear ()
{
...
namespace ipc
{
	futurehead::error read_access_config_toml (boost::filesystem::path const & data_path_a, futurehead::ipc::access & config_a)
	{
		futurehead::error error;
		auto toml_config_path = futurehead::get_access_toml_config_path (data_path_a);

...
void futurehead::confirmation_height_unbounded::process ()
{
	if (pending_empty ())
	{
		clear_process_vars ();
	...
	std::vector<futurehead::block_hash> orig_block_callback_data;
	...
	do
	{
		...
		std::vector<futurehead::block_hash> block_callback_datas_required;
...
void futurehead::confirmation_height_unbounded::collect_unconfirmed_receive_and_sources_for_account (uint64_t block_height_a, uint64_t confirmation_height_a, futurehead::block_hash const & hash_a, futurehead::account const & account_a, futurehead::read_transaction const & transaction_a, std::vector<receive_source_pair> & receive_source_pairs_a, std::vector<futurehead::block_hash> & block_callback_data_a, std::vector<futurehead::block_hash> & orig_block_callback_data_a)
{
	auto hash (hash_a);
	...
	while ((num_to_confirm > 0) && !hash.is_zero () && !stopped)
	{
		...
		if (block)
		{
			...
			if (!source.is_zero () && !ledger.is_epoch_link (source) && ledger.store.source_exists (transaction_a, source))
			{
				...
				receive_source_pairs_a.emplace_back (std::make_shared<conf_height_details> (account_a, hash, block_height, 1, std::vector<futurehead::block_hash>{ hash }), source);
			}
...
void futurehead::confirmation_height_unbounded::prepare_iterated_blocks_for_cementing (preparation_data & preparation_data_a)
{
 ...
 */
void futurehead::confirmation_height_unbounded::cement_blocks (futurehead::write_guard & scoped_write_guard_a)
{
	futurehead::timer<std::chrono::milliseconds> cemented_batch_timer;
	...
	auto error = false;
	{
		...
		while (!pending_writes.empty ())
		{
			...
			if (!error && pending.height > confirmation_height)
			{
				...
				ledger.stats.add (futurehead::stat::type::confirmation_height, futurehead::stat::detail::blocks_confirmed, futurehead::stat::dir::in, pending.height - confirmation_height);
				ledger.stats.add (futurehead::stat::type::confirmation_height, futurehead::stat::detail::blocks_confirmed_unbounded, futurehead::stat::dir::in, pending.height - confirmation_height);
...
std::shared_ptr<futurehead::block> futurehead::confirmation_height_unbounded::get_block_and_sideband (futurehead::block_hash const & hash_a, futurehead::transaction const & transaction_a)
{
...
bool futurehead::confirmation_height_unbounded::pending_empty () const
{
	return pending_writes.empty ();
}

void futurehead::confirmation_height_unbounded::clear_process_vars ()
{
...
futurehead::confirmation_height_unbounded::conf_height_details::conf_height_details (futurehead::account const & account_a, futurehead::block_hash const & hash_a, uint64_t height_a, uint64_t num_blocks_confirmed_a, std::vector<futurehead::block_hash> const & block_callback_data_a) :
account (account_a),
hash (hash_a),
height (height_a),
...
futurehead::confirmation_height_unbounded::receive_source_pair::receive_source_pair (std::shared_ptr<conf_height_details> const & receive_details_a, const block_hash & source_a) :
receive_details (receive_details_a),
source_hash (source_a)
{
}

futurehead::confirmation_height_unbounded::confirmed_iterated_pair::confirmed_iterated_pair (uint64_t confirmed_height_a, uint64_t iterated_height_a) :
confirmed_height (confirmed_height_a),
iterated_height (iterated_height_a)
{
}

std::unique_ptr<futurehead::container_info_component> futurehead::collect_container_info (confirmation_height_unbounded & confirmation_height_unbounded, const std::string & name_a)
{

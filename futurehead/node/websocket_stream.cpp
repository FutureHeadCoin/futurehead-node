...
class stream_wrapper : public futurehead::websocket::websocket_stream_concept
{
public:
	...
	void async_write (futurehead::shared_const_buffer const & buffer_a, std::function<void(boost::system::error_code, std::size_t)> callback_a) override
	{
...
#ifdef FUTUREHEAD_SECURE_RPC
futurehead::websocket::stream::stream (socket_type socket_a, boost::asio::ssl::context & ctx_a)
{
	impl = std::make_unique<stream_wrapper<wss_type>> (std::move (socket_a), ctx_a);
}
#endif
futurehead::websocket::stream::stream (socket_type socket_a)
{
	impl = std::make_unique<stream_wrapper<ws_type>> (std::move (socket_a));
}

[[nodiscard]] boost::asio::strand<boost::asio::io_context::executor_type> & futurehead::websocket::stream::get_strand ()
{
	return impl->get_strand ();
}

void futurehead::websocket::stream::handshake (std::function<void(boost::system::error_code const & ec)> callback_a)
{
	impl->handshake (callback_a);
}

void futurehead::websocket::stream::close (boost::beast::websocket::close_reason const & reason_a, boost::system::error_code & ec_a)
{
	impl->close (reason_a, ec_a);
}

void futurehead::websocket::stream::async_write (futurehead::shared_const_buffer const & buffer_a, std::function<void(boost::system::error_code, std::size_t)> callback_a)
{
	impl->async_write (buffer_a, callback_a);
}

void futurehead::websocket::stream::async_read (boost::beast::multi_buffer & buffer_a, std::function<void(boost::system::error_code, std::size_t)> callback_a)
{

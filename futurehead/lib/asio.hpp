...
#include <futurehead/boost/asio/write.hpp>
...
async_write (AsyncWriteStream & s, futurehead::shared_const_buffer const & buffer, WriteHandler && handler)
{
	return boost::asio::async_write (s, buffer, std::move (handler));
}

/**
 * Alternative to futurehead::async_write where scatter/gather is desired for best performance, and where

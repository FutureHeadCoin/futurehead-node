...
futurehead::error futurehead::rpc_secure_config::serialize_json (futurehead::jsonconfig & json) const
{
...
futurehead::error futurehead::rpc_secure_config::deserialize_json (futurehead::jsonconfig & json)
{
...
futurehead::error futurehead::rpc_secure_config::serialize_toml (futurehead::tomlconfig & toml) const
{
...
futurehead::error futurehead::rpc_secure_config::deserialize_toml (futurehead::tomlconfig & toml)
{
...
futurehead::rpc_config::rpc_config (uint16_t port_a, bool enable_control_a) :
address (boost::asio::ip::address_v6::loopback ().to_string ()),
port (port_a),
...
futurehead::error futurehead::rpc_config::serialize_json (futurehead::jsonconfig & json) const
{
	...
	futurehead::jsonconfig rpc_process_l;
...
futurehead::error futurehead::rpc_config::deserialize_json (bool & upgraded_a, futurehead::jsonconfig & json)
{
	if (!json.empty ())
	{
...
futurehead::error futurehead::rpc_config::serialize_toml (futurehead::tomlconfig & toml) const
{
	...
	futurehead::tomlconfig rpc_process_l;
	...
	futurehead::tomlconfig rpc_logging_l;
	rpc_logging_l.put ("log_rpc", rpc_logging.log_rpc, "Whether to log RPC calls.\ntype:bool");
	toml.put_child ("logging", rpc_logging_l);
	return toml.get_error ();
}

futurehead::error futurehead::rpc_config::deserialize_toml (futurehead::tomlconfig & toml)
{
	if (!toml.empty ())
	{
		...
		if (rpc_secure_l)
		{
			return futurehead::error ("The RPC secure configuration has moved to config-tls.toml. Please update the configuration.");
		}
...
futurehead::rpc_process_config::rpc_process_config () :
...
{
futurehead::error read_rpc_config_toml (boost::filesystem::path const & data_path_a, futurehead::rpc_config & config_a, std::vector<std::string> const & config_overrides)
{
	futurehead::error error;
	auto json_config_path = futurehead::get_rpc_config_path (data_path_a);
	auto toml_config_path = futurehead::get_rpc_toml_config_path (data_path_a);
	if (boost::filesystem::exists (json_config_path))
	{
		...
		{
			// Migrate
			futurehead::rpc_config config_json_l;
			...
			if (!error)
			{
				...
				futurehead::rpc_config config_defaults_l;
...
futurehead::error read_and_update_rpc_config (boost::filesystem::path const & data_path, futurehead::rpc_config & config_a)
{
	...
	auto config_path = futurehead::get_rpc_config_path (data_path);
...
std::string get_default_rpc_filepath ()
{
	...
	// Construct the futurehead_rpc executable file path based on where the currently running executable is found.

...
REGISTER_ERROR_CODES (futurehead, error_common);
REGISTER_ERROR_CODES (futurehead, error_blocks);
REGISTER_ERROR_CODES (futurehead, error_rpc);
REGISTER_ERROR_CODES (futurehead, error_process);
REGISTER_ERROR_CODES (futurehead, error_config);
...
public:
	error () = default;
	error (futurehead::error const & error_a) = default;
	error (futurehead::error && error_a) = default;
	...
	error & operator= (futurehead::error const & err_a);
	error & operator= (futurehead::error && err_a);
	...
	error & then (std::function<futurehead::error &()> next);
	...
	error & set (std::string message_a, std::error_code code_a = futurehead::error_common::generic);
...
/**
 * A type that manages a futurehead::error.
 * The default return type is futurehead::error&, though shared_ptr<futurehead::error> is a good option in cases
 * where shared error state is desirable.
 */
template <typename RET_TYPE = futurehead::error &>
class error_aware
{
	static_assert (std::is_same<RET_TYPE, futurehead::error &>::value || std::is_same<RET_TYPE, std::shared_ptr<futurehead::error>>::value, "Must be futurehead::error& or shared_ptr<futurehead::error>");

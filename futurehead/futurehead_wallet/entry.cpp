...
#include <futurehead/futurehead_wallet/icon.hpp>
...
{
	QMessageBox message (QMessageBox::Critical, "Error starting Futurehead", message_a.c_str ());
...
{
	QMessageBox message (QMessageBox::NoIcon, "Help", "see <a href=\"https://docs.futurehead.org/commands/command-line-interface/#launch-options\">launch options</a> ");
...
futurehead::error write_wallet_config (futurehead::wallet_config & config_a, boost::filesystem::path const & data_path_a)
{
	futurehead::tomlconfig wallet_config_toml;
	auto wallet_path (futurehead::get_qtwallet_toml_config_path (data_path_a));
...
futurehead::error read_wallet_config (futurehead::wallet_config & config_a, boost::filesystem::path const & data_path_a)
{
	futurehead::tomlconfig wallet_config_toml;
	auto wallet_path (futurehead::get_qtwallet_toml_config_path (data_path_a));
...
int run_wallet (QApplication & application, int argc, char * const * argv, boost::filesystem::path const & data_path, futurehead::node_flags const & flags)
{
	int result (0);
	...
	{
		error = futurehead::error_config::rocksdb_enabled_but_not_supported;
	}
#endif
	...
	if (!error)
	{
		...
		futurehead::thread_runner runner (io_ctx, config.node.io_threads);

		std::shared_ptr<futurehead::node> node;
		std::shared_ptr<futurehead_qt::wallet> gui;
		futurehead::set_application_icon (application);
		auto opencl (futurehead::opencl_work::create (config.opencl_enable, config.opencl, logger));
		futurehead::work_pool work (config.node.work_threads, config.node.pow_sleep_interval, opencl ? [&opencl](futurehead::work_version const version_a, futurehead::root const & root_a, uint64_t difficulty_a, std::atomic<int> &) {
		...
		node = std::make_shared<futurehead::node> (io_ctx, data_path, alarm, config.node, work, flags);
		if (!node->init_error ())
		{
			...
			node->start ();
			futurehead::ipc::ipc_server ipc (*node, config.rpc);
			...
			if (config.pow_server.enable)
			{
				...
				{
					splash->hide ();
					show_error (std::string ("futurehead_pow_server is configured to start as a child process, however the file cannot be found at: ") + config.pow_server.pow_server_path);
					std::exit (1);
				}
...
#else
				splash->hide ();
				show_error ("futurehead_pow_server is configured to start as a child process, but this is not supported on this system. Disable startup and start the server manually.");
				std::exit (1);
#endif
			}
			...
			if (config.rpc_enable)
			{
				if (!config.rpc.child_process.enable)
				{
					...
					rpc_handler = std::make_unique<futurehead::inprocess_rpc_handler> (*node, ipc, config.rpc);
			...
			QObject::connect (&application, &QApplication::aboutToQuit, [&]() {
				...
				if (futurehead_pow_server_process)
				{
					futurehead_pow_server_process->terminate ();
				}
#endif
				runner.stop_event_processing ();
			});
			application.postEvent (&processor, new futurehead_qt::eventloop_event ([&]() {
				gui = std::make_shared<futurehead_qt::wallet> (application, processor, *node, wallet, wallet_config.account);
				splash->close ();
				gui->start ();
...
int main (int argc, char * const * argv)
{
	...
	try
	{
		...
		if (network != vm.end ())
		{
			...
			if (err)
			{
				show_error (futurehead::network_constants::active_network_err_msg);
				std::exit (1);
			}
		}
		...
		if (ec == futurehead::error_cli::unknown_command)
		{
			...
			else
			{
				try
				{
					...
					{
						data_path = futurehead::working_path ();
					}

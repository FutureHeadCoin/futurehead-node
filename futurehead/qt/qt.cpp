...
bool futurehead_qt::eventloop_processor::event (QEvent * event_a)
{
	debug_assert (dynamic_cast<futurehead_qt::eventloop_event *> (event_a) != nullptr);
	static_cast<futurehead_qt::eventloop_event *> (event_a)->action ();
	return true;
}

futurehead_qt::eventloop_event::eventloop_event (std::function<void()> const & action_a) :
QEvent (QEvent::Type::User),
action (action_a)
{
}

futurehead_qt::self_pane::self_pane (futurehead_qt::wallet & wallet_a, futurehead::account const & account_a) :
window (new QWidget),
layout (new QVBoxLayout),
self_layout (new QHBoxLayout),
self_window (new QWidget),
your_account_label (new QLabel ("Your Futurehead account:")),
...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("%1% %2% network") % FUTUREHEAD_VERSION_STRING % network).c_str ());
...
void futurehead_qt::self_pane::set_balance_text (std::pair<futurehead::uint128_t, futurehead::uint128_t> balance_a)
{
...
futurehead_qt::accounts::accounts (futurehead_qt::wallet & wallet_a) :
...
void futurehead_qt::accounts::refresh_wallet_balance ()
{
	auto transaction (this->wallet.wallet_m->wallets.tx_begin_read ());
...
void futurehead_qt::accounts::refresh ()
{
	...
	{
		futurehead::public_key key (i->first);
...
futurehead_qt::import::import (futurehead_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
futurehead_qt::history::history (futurehead::ledger & ledger_a, futurehead::account const & account_a, futurehead_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
model (new QStandardItemModel),
view (new QTableView),
...
namespace
{
class short_text_visitor : public futurehead::block_visitor
{
public:
	short_text_visitor (futurehead::transaction const & transaction_a, futurehead::ledger & ledger_a) :
...
void futurehead_qt::history::refresh ()
{
...
futurehead_qt::block_viewer::block_viewer (futurehead_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void futurehead_qt::block_viewer::rebroadcast_action (futurehead::block_hash const & hash_a)
{
	auto done (true);
...
futurehead_qt::account_viewer::account_viewer (futurehead_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
wallet (wallet_a)
{
	...
	QObject::connect (refresh, &QPushButton::released, [this]() {
		...
		if (!account.decode_account (account_line->text ().toStdString ()))
		{
			...
			auto final_text (std::string ("Balance (FUTUREHEAD): ") + wallet.format_balance (balance.first));
...
futurehead_qt::stats_viewer::stats_viewer (futurehead_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
refresh (new QPushButton ("Refresh")),
...
void futurehead_qt::stats_viewer::refresh_stats ()
{
	model->removeRows (0, model->rowCount ());
...
futurehead_qt::status::status (futurehead_qt::wallet & wallet_a) :
wallet (wallet_a)
{
	wallet.status->setToolTip ("Wallet status, block count (blocks downloaded)");
	active.insert (futurehead_qt::status_types::nominal);
	set_text ();
}

void futurehead_qt::status::erase (futurehead_qt::status_types status_a)
{
	debug_assert (status_a != futurehead_qt::status_types::nominal);
	auto erased (active.erase (status_a));
	(void)erased;
	set_text ();
}

void futurehead_qt::status::insert (futurehead_qt::status_types status_a)
{
	debug_assert (status_a != futurehead_qt::status_types::nominal);
...
std::string futurehead_qt::status::text ()
{
	...
	switch (*active.begin ())
	{
		case futurehead_qt::status_types::disconnected:
			result = "Status: Disconnected";
			break;
		case futurehead_qt::status_types::working:
			result = "Status: Generating proof of work";
			break;
		case futurehead_qt::status_types::synchronizing:
			result = "Status: Synchronizing";
			break;
		case futurehead_qt::status_types::locked:
			result = "Status: Wallet locked";
			break;
		case futurehead_qt::status_types::vulnerable:
			result = "Status: Wallet password empty";
			break;
		case futurehead_qt::status_types::active:
			result = "Status: Wallet active";
			break;
		case futurehead_qt::status_types::nominal:
...
std::string futurehead_qt::status::color ()
{
	debug_assert (!active.empty ());
	std::string result;
	switch (*active.begin ())
...
futurehead_qt::wallet::wallet (QApplication & application_a, futurehead_qt::eventloop_processor & processor_a, futurehead::node & node_a, std::shared_ptr<futurehead::wallet> wallet_a, futurehead::account & account_a) :
rendering_ratio (futurehead::Mxrb_ratio),
node (node_a),
wallet_m (wallet_a),
account (account_a),
...
void futurehead_qt::wallet::ongoing_refresh ()
{
	std::weak_ptr<futurehead_qt::wallet> wallet_w (shared_from_this ());
...
void futurehead_qt::wallet::start ()
{
	ongoing_refresh ();
	std::weak_ptr<futurehead_qt::wallet> this_w (shared_from_this ());
					...
					if (actual <= balance)
					{
						...
						if (this_l->wallet_m->store.valid_password (transaction))
						{
							...
							this_l->node.background ([this_w, account_l, actual]() {
								if (auto this_l = this_w.lock ())
								{
									this_l->wallet_m->send_async (this_l->account, account_l, actual, [this_w](std::shared_ptr<futurehead::block> block_a) {
										if (auto this_l = this_w.lock ())
										{
	...
	node.observers.blocks.add ([this_w](futurehead::election_status const & status_a, futurehead::account const & account_a, futurehead::uint128_t const & amount_a, bool) {
		if (auto this_l = this_w.lock ())
		{
	...
	node.observers.account_balance.add ([this_w](futurehead::account const & account_a, bool is_pending) {
		if (auto this_l = this_w.lock ())
		{
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (futurehead_qt::status_types::active);
					}
					else
					{
						this_l->active_status.erase (futurehead_qt::status_types::active);
					}
				}
			}));
		}
	});
	node.observers.endpoint.add ([this_w](std::shared_ptr<futurehead::transport::channel>) {
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w]() {
				if (auto this_l = this_w.lock ())
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, active_a]() {
				if (auto this_l = this_w.lock ())
				{
					if (active_a)
					{
						this_l->active_status.insert (futurehead_qt::status_types::synchronizing);
					}
					else
					{
						this_l->active_status.erase (futurehead_qt::status_types::synchronizing);
		...
		if (auto this_l = this_w.lock ())
		{
			this_l->application.postEvent (&this_l->processor, new eventloop_event ([this_w, working]() {
				if (auto this_l = this_w.lock ())
				{
					if (working)
					{
						this_l->active_status.insert (futurehead_qt::status_types::working);
					}
					else
					{
						this_l->active_status.erase (futurehead_qt::status_types::working);
					}
				}
			}));
		}
...
void futurehead_qt::wallet::refresh ()
{
	{
...
void futurehead_qt::wallet::update_connected ()
{
	if (node.network.empty ())
	{
		active_status.insert (futurehead_qt::status_types::disconnected);
	}
	else
	{
		active_status.erase (futurehead_qt::status_types::disconnected);
	}
}

void futurehead_qt::wallet::empty_password ()
{
...
void futurehead_qt::wallet::change_rendering_ratio (futurehead::uint128_t const & rendering_ratio_a)
{
...
//TO-CHANGE
std::string futurehead_qt::wallet::format_balance (futurehead::uint128_t const & balance) const
{
	auto balance_str = futurehead::amount (balance).format_balance (rendering_ratio, 3, false);
	auto unit = std::string ("FPSC");
	if (rendering_ratio == futurehead::kxrb_ratio)
	{
		unit = std::string ("kfpsc");
	}
	else if (rendering_ratio == futurehead::xrb_ratio)
	{
		unit = std::string ("fpsc");
	}
	else if (rendering_ratio == futurehead::raw_ratio)
	{
		unit = std::string ("raw");
	}
...
void futurehead_qt::wallet::push_main_stack (QWidget * widget_a)
{
	main_stack->addWidget (widget_a);
	main_stack->setCurrentIndex (main_stack->count () - 1);
}

void futurehead_qt::wallet::pop_main_stack ()
{
	main_stack->removeWidget (main_stack->currentWidget ());
}

futurehead_qt::settings::settings (futurehead_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
password (new QLineEdit),
...
wallet (wallet_a)
{
	...
	//TO-CHANGE Coment to hide representative
	...
	QObject::connect (change_rep, &QPushButton::released, [this]() {
		futurehead::account representative_l;
...
void futurehead_qt::settings::refresh_representative ()
{
...
void futurehead_qt::settings::activate ()
{
	this->wallet.push_main_stack (window);
}

void futurehead_qt::settings::update_locked (bool invalid, bool vulnerable)
{
	if (invalid)
	{
		this->wallet.active_status.insert (futurehead_qt::status_types::locked);
	}
	else
	{
		this->wallet.active_status.erase (futurehead_qt::status_types::locked);
	}
	if (vulnerable)
	{
		this->wallet.active_status.insert (futurehead_qt::status_types::vulnerable);
	}
	else
	{
		this->wallet.active_status.erase (futurehead_qt::status_types::vulnerable);
	}
}

futurehead_qt::advanced_actions::advanced_actions (futurehead_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
mfuturehead_unit (new QRadioButton ("mfpsc")),//TO-CHANGE
kfuturehead_unit (new QRadioButton ("kfpsc")),//TO-CHANGE
futurehead_unit (new QRadioButton ("fpsc")),//TO-CHANGE
raw_unit (new QRadioButton ("raw")),//TO-CHANGE
back (new QPushButton ("Back")),
...
bootstrap_label (new QLabel ("IPV6:port \"::ffff:192.168.0.1:7155\"")), //TO-CHANGE
...
wallet (wallet_a)
{
	ratio_group->addButton (mfuturehead_unit);
	ratio_group->addButton (kfuturehead_unit);
	ratio_group->addButton (futurehead_unit);
	ratio_group->addButton (raw_unit);
	ratio_group->setId (mfuturehead_unit, 0);
	ratio_group->setId (kfuturehead_unit, 1);
	ratio_group->setId (futurehead_unit, 2);
	ratio_group->setId (raw_unit, 3);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (mfuturehead_unit);
	scale_layout->addWidget (kfuturehead_unit);
	scale_layout->addWidget (futurehead_unit);
	...
	QObject::connect (mfuturehead_unit, &QRadioButton::toggled, [this]() {
		if (mfuturehead_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (mfuturehead_unit));
			this->wallet.change_rendering_ratio (futurehead::Mxrb_ratio);
		}
	});
	QObject::connect (kfuturehead_unit, &QRadioButton::toggled, [this]() {
		if (kfuturehead_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (kfuturehead_unit));
			this->wallet.change_rendering_ratio (futurehead::kxrb_ratio);
		}
	});
	QObject::connect (futurehead_unit, &QRadioButton::toggled, [this]() {
		if (futurehead_unit->isChecked ())
		{
			QSettings ().setValue (saved_ratio_key, ratio_group->id (futurehead_unit));
			this->wallet.change_rendering_ratio (futurehead::xrb_ratio);
		}
	});
	QObject::connect (raw_unit, &QRadioButton::toggled, [this]() {
		if (raw_unit->isChecked ())
		{
			...
			this->wallet.change_rendering_ratio (futurehead::raw_ratio);
		}
	});
	auto selected_ratio_id (QSettings ().value (saved_ratio_key, ratio_group->id (mfuturehead_unit)).toInt ());
	...
	QObject::connect (peers_bootstrap, &QPushButton::released, [this]() {
		...
		auto error (futurehead::parse_endpoint (bootstrap_line->text ().toStdString (), endpoint));
		if (!error)
		{
...
void futurehead_qt::advanced_actions::refresh_peers ()
{
...
void futurehead_qt::advanced_actions::refresh_ledger ()
{
	...
	for (auto i (wallet.node.ledger.store.latest_begin (transaction)), j (wallet.node.ledger.store.latest_end ()); i != j; ++i)
	{
		...
		std::string balance;
		futurehead::amount (info.balance.number () / wallet.rendering_ratio).encode_dec (balance);
...
void futurehead_qt::advanced_actions::refresh_stats ()
{
	wallet.stats_viewer.refresh_stats ();
}

futurehead_qt::block_entry::block_entry (futurehead_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
block (new QPlainTextEdit),
status (new QLabel),
...
wallet (wallet_a)
{
	...
	QObject::connect (process, &QPushButton::released, [this]() {
		...
		try
		{
			...
			auto block_l (futurehead::deserialize_block_json (tree));
			if (block_l != nullptr)
			{
				show_label_ok (*status);
				this->status->setText ("");
				if (!futurehead::work_validate_entry (*block_l))
				{
...
futurehead_qt::block_creation::block_creation (futurehead_qt::wallet & wallet_a) :
window (new QWidget),
layout (new QVBoxLayout),
...
void futurehead_qt::block_creation::deactivate_all ()
{
	account_label->hide ();
	account->hide ();
	source_label->hide ();
...
void futurehead_qt::block_creation::activate_send ()
{
	account_label->show ();
	account->show ();
	amount_label->show ();
...
void futurehead_qt::block_creation::activate_receive ()
{
	source_label->show ();
	source->show ();
}

void futurehead_qt::block_creation::activate_open ()
{
	source_label->show ();
	source->show ();
...
void futurehead_qt::block_creation::activate_change ()
{
	account_label->show ();
	account->show ();
	representative_label->show ();
			...
			if (!error)
			{
				...
				if (!wallet.wallet_m->store.fetch (transaction, account_l, key))
				{
					...
					if (amount_l.number () <= balance)
					{
						...
						(void)error;
						debug_assert (!error);
						futurehead::state_block send (account_l, info.head, info.representative, balance - amount_l.number (), destination_l, key, account_l, 0);
...
void futurehead_qt::block_creation::create_receive ()
{
	futurehead::block_hash source_l;
	...
	if (!error)
	{
		...
		if (block_l != nullptr)
		{
			...
			{
				futurehead::pending_key pending_key (destination, source_l);
				...
				if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
				{
					...
					if (!error)
					{
						...
						if (!error)
						{
							futurehead::state_block receive (pending_key.account, info.head, info.representative, info.balance.number () + pending.amount.number (), source_l, key, pending_key.account, 0);
...
void futurehead_qt::block_creation::create_change ()
{
	futurehead::account account_l;
	auto error (account_l.decode_account (account->text ().toStdString ()));
	if (!error)
	{
		...
		if (!error)
		{
			...
			if (!error)
			{
				...
				if (!error)
				{
					futurehead::state_block change (account_l, info.head, representative_l, info.balance, 0, key, account_l, 0);
...
void futurehead_qt::block_creation::create_open ()
{
	futurehead::block_hash source_l;
	auto error (source_l.decode_hex (source->text ().toStdString ()));
	if (!error)
				...
				if (!destination.is_zero ())
				{
					...
					if (!wallet.node.store.pending_get (block_transaction, pending_key, pending))
					{
						...
						if (error)
						{
							...
							if (!error)
							{
								futurehead::state_block open (pending_key.account, 0, representative_l, pending.amount, source_l, key, pending_key.account, 0);
